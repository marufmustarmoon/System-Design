...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি কমায় ল্যাটেন্সি জন্য global ইউজাররা এবং offloads origin infrastructure।
...
এই জায়গায় `CDN Caching` সামনে আসে। সহজ ভাষায়, CDN caching stores responses at edge locations close to users।
...
- Right: `CDN Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`CDN Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Static assets, public APIs, cacheable dynamic pages সাথে correct keys।
- business signal: এটি কমায় ল্যাটেন্সি জন্য global ইউজাররা এবং offloads origin infrastructure।
...
`CDN Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Per-ইউজার private রেসপন্সগুলো unless explicitly designed জন্য safe edge caching।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Caching authenticated রেসপন্সগুলো at the edge ছাড়া key segregation।
...
- ধাপ ২: `CDN Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `CDN Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `CDN Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: CDN caching stores responses at edge locations close to users।
...
- business impact: এটি কমায় ল্যাটেন্সি জন্য global ইউজাররা এবং offloads origin infrastructure।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `CDN Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `CDN Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Caching authenticated রেসপন্সগুলো at the edge ছাড়া key segregation।
...
- `CDN Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
