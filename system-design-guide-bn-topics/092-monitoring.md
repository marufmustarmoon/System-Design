...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, আপনি পারে না operate অথবা উন্নত করতে ডিস্ট্রিবিউটেড সিস্টেম reliably ছাড়া visibility।
...
এই জায়গায় `Monitoring` সামনে আসে। সহজ ভাষায়, Monitoring is the practice of collecting signals about system health, পারফরম্যান্স, usage, and failures over time।
...
- Right: `Monitoring` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Monitoring` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: সবসময় জন্য production সিস্টেমগুলো; describe what আপনি will measure এবং alert on।
- business signal: আপনি পারে না operate অথবা উন্নত করতে ডিস্ট্রিবিউটেড সিস্টেম reliably ছাড়া visibility।
...
`Monitoring` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না উপর-ইনডেক্স on vanity মেট্রিকস যা do না drive action।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: কোনো mention of মনিটরিং, dashboards, অথবা অ্যালার্ট in a production architecture answer।
...
- ধাপ ২: `Monitoring` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Monitoring` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: SLI/SLO, metrics/logs/traces, alerts, dashboards, incident response।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Monitoring` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Monitoring is the practice of collecting signals about system health, পারফরম্যান্স, usage, and failures over time।
...
- business impact: আপনি পারে না operate অথবা উন্নত করতে ডিস্ট্রিবিউটেড সিস্টেম reliably ছাড়া visibility।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Monitoring` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Monitoring` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: কোনো mention of মনিটরিং, dashboards, অথবা অ্যালার্ট in a production architecture answer।
...
- `Monitoring` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
