...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি provides a full অডিট trail, temporal history, এবং replay capability জন্য complex business workflows।
...
এই জায়গায় `Event Sourcing` সামনে আসে। সহজ ভাষায়, Event sourcing stores state changes as an append-only sequence of events, and current state is reconstructed from those events।
...
- Right: `Event Sourcing` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Event Sourcing` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Auditable workflows, temporal queries, complex domain স্টেট machines।
- business signal: এটি provides a full অডিট trail, temporal history, এবং replay capability জন্য complex business workflows।
...
`Event Sourcing` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Simple CRUD সিস্টেমগুলো সাথে no replay/history need।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Choosing ইভেন্ট সোর্সিং জন্য a simple CRUD app "কারণ it হলো scalable."।
...
- ধাপ ২: `Event Sourcing` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Event Sourcing` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: append-only log, event replay, projection rebuild, audit trail, versioned events।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Event Sourcing` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Event sourcing stores state changes as an append-only sequence of events, and current state is reconstructed from those events।
...
- business impact: এটি provides a full অডিট trail, temporal history, এবং replay capability জন্য complex business workflows।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Event Sourcing` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- `Uber, Amazon`-এর মতো সিস্টেমে একই pattern সব feature-এ একভাবে চলে না; context অনুযায়ী প্রয়োগ বদলায়।
- তাই `Event Sourcing` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Choosing ইভেন্ট সোর্সিং জন্য a simple CRUD app "কারণ it হলো scalable."।
...
- `Event Sourcing` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
