...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি simplifies write ordering এবং conflict handling।
...
এই জায়গায় `Master-Slave` সামনে আসে। সহজ ভাষায়, Master-slave (primary-replica) রেপ্লিকেশন uses one writable node and one or more read-only replicas।
...
- Right: `Master-Slave` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Master-Slave` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Read-heavy workloads সাথে moderate write volume এবং clear primary ওনারশিপ।
- business signal: এটি simplifies write ordering এবং conflict handling।
...
`Master-Slave` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Multi-রিজিয়ন write-heavy সিস্টেমগুলো needing local writes everywhere।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Sending all reads to replicas ছাড়া discussing read-পরে-write guarantees।
...
- ধাপ ২: `Master-Slave` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Master-Slave` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: master, slave, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Master-Slave` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Master-slave (primary-replica) রেপ্লিকেশন uses one writable node and one or more read-only replicas।
...
- business impact: এটি simplifies write ordering এবং conflict handling।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Master-Slave` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Master-Slave` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Sending all reads to replicas ছাড়া discussing read-পরে-write guarantees।
...
- `Master-Slave` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
