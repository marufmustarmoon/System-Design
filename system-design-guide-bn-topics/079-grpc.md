...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি উন্নত করে efficiency এবং developer ergonomics জন্য internal সার্ভিস-টু-সার্ভিস APIs at scale।
...
এই জায়গায় `gRPC` সামনে আসে। সহজ ভাষায়, gRPC is a high-পারফরম্যান্স RPC framework using Protocol Buffers and HTTP/2, with strong typing and streaming support।
...
- Right: `gRPC` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`gRPC` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Internal microservices, low-ল্যাটেন্সি paths, streaming, strongly typed contracts।
- business signal: এটি উন্নত করে efficiency এবং developer ergonomics জন্য internal সার্ভিস-টু-সার্ভিস APIs at scale।
...
`gRPC` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Public APIs needing easy curl/browser access এবং broad third-party সাপোর্ট।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Choosing gRPC ছাড়া a plan জন্য observability, versioning, এবং external compatibility।
...
- ধাপ ২: `gRPC` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `gRPC` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: IDL/protobuf, deadlines/timeouts, streaming, backward compatibility, service-to-service।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `gRPC` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: gRPC is a high-পারফরম্যান্স RPC framework using Protocol Buffers and HTTP/2, with strong typing and streaming support।
...
- business impact: এটি উন্নত করে efficiency এবং developer ergonomics জন্য internal সার্ভিস-টু-সার্ভিস APIs at scale।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `gRPC` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- `Netflix, Google`-এর মতো সিস্টেমে একই pattern সব feature-এ একভাবে চলে না; context অনুযায়ী প্রয়োগ বদলায়।
- তাই `gRPC` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Choosing gRPC ছাড়া a plan জন্য observability, versioning, এবং external compatibility।
...
- `gRPC` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
