...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, ইউজাররা still need feedback, even যখন কাজ continues পরে the initial রিকোয়েস্ট returns।
...
এই জায়গায় `Returning Results` সামনে আসে। সহজ ভাষায়, Returning results for background work means how clients learn task status and retrieve outputs after async processing।
...
- Right: `Returning Results` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Returning Results` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: লং-রানিং tasks like exports, video processing, এবং bulk imports।
- business signal: ইউজাররা still need feedback, even যখন কাজ continues পরে the initial রিকোয়েস্ট returns।
...
`Returning Results` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Short operations যা পারে complete within normal রিকোয়েস্ট ল্যাটেন্সি budgets।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "async" ছাড়া a ক্লায়েন্ট-visible status/result retrieval design।
...
- ধাপ ২: `Returning Results` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Returning Results` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: returning, results, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Returning Results` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Returning results for background work means how clients learn task status and retrieve outputs after async processing।
...
- business impact: ইউজাররা still need feedback, even যখন কাজ continues পরে the initial রিকোয়েস্ট returns।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Returning Results` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Returning Results` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "async" ছাড়া a ক্লায়েন্ট-visible status/result retrieval design।
...
- `Returning Results` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
