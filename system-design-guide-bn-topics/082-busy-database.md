...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, ডাটাবেজগুলো become the default integration point জন্য many features, causing contention এবং slow queries এর অধীনে লোড।
...
এই জায়গায় `Busy Database` সামনে আসে। সহজ ভাষায়, A busy ডাটাবেজ antipattern occurs when too much application work depends on one overloaded ডাটাবেজ।
...
- Right: `Busy Database` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Busy Database` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: As a warning যখন many সার্ভিসগুলো sync-read/write the same DB in hot paths।
- business signal: ডাটাবেজগুলো become the default integration point জন্য many features, causing contention এবং slow queries এর অধীনে লোড।
...
`Busy Database` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না declare DB bottleneck ছাড়া evidence; the issue may হতে app logic অথবা network।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Jumping to শার্ডিং আগে checking ইনডেক্সগুলো এবং query plans।
...
- ধাপ ২: `Busy Database` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Busy Database` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: data model, query pattern, indexing, consistency, scale trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Busy Database` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: A busy ডাটাবেজ antipattern occurs when too much application work depends on one overloaded ডাটাবেজ।
...
- business impact: ডাটাবেজগুলো become the default integration point জন্য many features, causing contention এবং slow queries এর অধীনে লোড।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Busy Database` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Busy Database` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Jumping to শার্ডিং আগে checking ইনডেক্সগুলো এবং query plans।
...
- `Busy Database` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
