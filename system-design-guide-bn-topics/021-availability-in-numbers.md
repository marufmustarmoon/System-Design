...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, টিমগুলো need measurable SLO targets, না vague claims like "highly available."।
...
এই জায়গায় `Availability in Numbers` সামনে আসে। সহজ ভাষায়, Availability in numbers translates uptime goals (like 99.9%) into allowed downtime over a period।
...
- Right: `Availability in Numbers` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Availability in Numbers` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন discussing SLOs, SLAs, এবং architecture খরচ/complexity ট্রেড-অফ।
- business signal: টিমগুলো need measurable SLO targets, না vague claims like "highly available."।
...
`Availability in Numbers` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না throw "five 9s" into every design ছাড়া business justification।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Quoting uptime percentages ছাড়া defining measurement scope।
...
- ধাপ ২: `Availability in Numbers` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Availability in Numbers` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: uptime target, downtime budget, failover, redundancy, dependency chain।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Availability in Numbers` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Availability in numbers translates uptime goals (like 99.9%) into allowed downtime over a period।
...
- business impact: টিমগুলো need measurable SLO targets, না vague claims like "highly available."।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Availability in Numbers` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Availability in Numbers` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Quoting uptime percentages ছাড়া defining measurement scope।
...
- `Availability in Numbers` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
