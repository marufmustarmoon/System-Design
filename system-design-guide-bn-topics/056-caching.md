...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি lowers ল্যাটেন্সি, কমায় ডাটাবেজ/origin লোড, এবং উন্নত করে থ্রুপুট।
...
এই জায়গায় `Caching` সামনে আসে। সহজ ভাষায়, Caching stores frequently used data in a faster layer (memory or edge) to reduce repeated expensive work।
...
- Right: `Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Read-heavy hot ডেটা, expensive computations, repeated API রেসপন্স।
- business signal: এটি lowers ল্যাটেন্সি, কমায় ডাটাবেজ/origin লোড, এবং উন্নত করে থ্রুপুট।
...
`Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Highly dynamic ডেটা সাথে low reuse অথবা correctness-sensitive paths যা পারে না tolerate staleness।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding a ক্যাশ ছাড়া defining TTL, key format, এবং ফেইলিউর behavior।
...
- ধাপ ২: `Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Caching stores frequently used data in a faster layer (memory or edge) to reduce repeated expensive work।
...
- business impact: এটি lowers ল্যাটেন্সি, কমায় ডাটাবেজ/origin লোড, এবং উন্নত করে থ্রুপুট।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adding a ক্যাশ ছাড়া defining TTL, key format, এবং ফেইলিউর behavior।
...
- `Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
