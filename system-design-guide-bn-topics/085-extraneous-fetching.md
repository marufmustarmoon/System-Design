...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি wastes bandwidth, CPU, memory, এবং ডাটাবেজ capacity, এবং increases ল্যাটেন্সি।
...
এই জায়গায় `Extraneous Fetching` সামনে আসে। সহজ ভাষায়, Extraneous fetching is retrieving data that the client or service does not actually need।
...
- Right: `Extraneous Fetching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Extraneous Fetching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন optimizing mobile ক্লায়েন্টগুলো অথবা high-QPS read APIs।
- business signal: এটি wastes bandwidth, CPU, memory, এবং ডাটাবেজ capacity, এবং increases ল্যাটেন্সি।
...
`Extraneous Fetching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না উপর-customize রেসপন্সগুলো জন্য every ক্লায়েন্ট যদি it creates unmaintainable API sprawl।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Returning full objects সাথে large nested fields by default on list endpoints।
...
- ধাপ ২: `Extraneous Fetching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Extraneous Fetching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: extraneous, fetching, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Extraneous Fetching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Extraneous fetching is retrieving data that the client or service does not actually need।
...
- business impact: এটি wastes bandwidth, CPU, memory, এবং ডাটাবেজ capacity, এবং increases ল্যাটেন্সি।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Extraneous Fetching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Extraneous Fetching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Returning full objects সাথে large nested fields by default on list endpoints।
...
- `Extraneous Fetching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
