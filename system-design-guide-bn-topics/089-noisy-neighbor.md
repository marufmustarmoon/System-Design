...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Shared infrastructure উন্নত করে utilization, but contention পারে break fairness এবং SLOs।
...
এই জায়গায় `Noisy Neighbor` সামনে আসে। সহজ ভাষায়, Noisy neighbor is when one workload/tenant consumes shared resources and degrades পারফরম্যান্স for others।
...
- Right: `Noisy Neighbor` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Noisy Neighbor` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Multi-tenant সিস্টেমগুলো, shared worker pools, shared DB clusters।
- business signal: Shared infrastructure উন্নত করে utilization, but contention পারে break fairness এবং SLOs।
...
`Noisy Neighbor` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না assume dedicated hardware হলো the শুধু fix; software isolation অনেক সময় কাজ করে first।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Shared resources সাথে no quotas অথবা admission control।
...
- ধাপ ২: `Noisy Neighbor` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Noisy Neighbor` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: noisy, neighbor, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Noisy Neighbor` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Noisy neighbor is when one workload/tenant consumes shared resources and degrades পারফরম্যান্স for others।
...
- business impact: Shared infrastructure উন্নত করে utilization, but contention পারে break fairness এবং SLOs।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Noisy Neighbor` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Noisy Neighbor` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Shared resources সাথে no quotas অথবা admission control।
...
- `Noisy Neighbor` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
