...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Networks fail, packets drop, এবং রিজিয়নগুলো disconnect।
...
এই জায়গায় `CAP Theorem` সামনে আসে। সহজ ভাষায়, CAP says that during a network partition, a ডিস্ট্রিবিউটেড সিস্টেম can choose at most one of Consistency or Availability while still tolerating the partition।
...
- Right: `CAP Theorem` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`CAP Theorem` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন discussing replica behavior, quorum choices, এবং পার্টিশন handling।
- business signal: Networks fail, packets drop, এবং রিজিয়নগুলো disconnect।
...
`CAP Theorem` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না ব্যবহার CAP to explain every ডাটাবেজ decision unrelated to পার্টিশনগুলো।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Claiming a distributed সিস্টেম হলো simultaneously CA in the presence of পার্টিশনগুলো।
...
- ধাপ ২: `CAP Theorem` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `CAP Theorem` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: partition tolerance, consistency vs availability, network partition, AP/CP trade-off, failure behavior।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `CAP Theorem` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: CAP says that during a network partition, a ডিস্ট্রিবিউটেড সিস্টেম can choose at most one of Consistency or Availability while still tolerating the partition।
...
- business impact: Networks fail, packets drop, এবং রিজিয়নগুলো disconnect।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `CAP Theorem` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `CAP Theorem` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Claiming a distributed সিস্টেম হলো simultaneously CA in the presence of পার্টিশনগুলো।
...
- `CAP Theorem` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
