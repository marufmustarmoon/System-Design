...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি পারে eliminate network round-trips entirely এবং উন্নত করতে perceived পারফরম্যান্স।
...
এই জায়গায় `Client Caching` সামনে আসে। সহজ ভাষায়, Client caching stores data on the user device/browser (memory, local storage, app ক্যাশ, HTTP ক্যাশ)।
...
- Right: `Client Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Client Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Static assets, ইউজার-tolerant metadata, immutable/versioned content।
- business signal: এটি পারে eliminate network round-trips entirely এবং উন্নত করতে perceived পারফরম্যান্স।
...
`Client Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Sensitive personalized ডেটা ছাড়া strong ক্যাশ controls এবং auth boundaries।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Ignoring stale ক্লায়েন্ট ক্যাশগুলো সময় ডিপ্লয়মেন্টগুলো।
...
- ধাপ ২: `Client Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Client Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Client Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Client caching stores data on the user device/browser (memory, local storage, app ক্যাশ, HTTP ক্যাশ)।
...
- business impact: এটি পারে eliminate network round-trips entirely এবং উন্নত করতে perceived পারফরম্যান্স।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Client Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Client Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Ignoring stale ক্লায়েন্ট ক্যাশগুলো সময় ডিপ্লয়মেন্টগুলো।
...
- `Client Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
