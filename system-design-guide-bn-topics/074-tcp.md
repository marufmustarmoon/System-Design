...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Many applications need ডেটা to arrive in order এবং ছাড়া loss, সাথে retransmissions handled by the network stack।
...
এই জায়গায় `TCP` সামনে আসে। সহজ ভাষায়, TCP is a transport protocol that provides reliable, ordered byte-stream delivery between endpoints।
...
- Right: `TCP` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`TCP` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Most APIs, ডাটাবেজগুলো, এবং সার্ভিস-টু-সার্ভিস calls needing reliable delivery।
- business signal: Many applications need ডেটা to arrive in order এবং ছাড়া loss, সাথে retransmissions handled by the network stack।
...
`TCP` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: রিয়েল-টাইম voice/video/gaming scenarios যা পারে tolerate some loss but need lower ল্যাটেন্সি।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Describing TCP as মেসেজ-based এর বদলে a byte stream।
...
- ধাপ ২: `TCP` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `TCP` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: ordered delivery, retransmission, connection-oriented, latency trade-off, reliability।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `TCP` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: TCP is a transport protocol that provides reliable, ordered byte-stream delivery between endpoints।
...
- business impact: Many applications need ডেটা to arrive in order এবং ছাড়া loss, সাথে retransmissions handled by the network stack।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `TCP` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `TCP` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Describing TCP as মেসেজ-based এর বদলে a byte stream।
...
- `TCP` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
