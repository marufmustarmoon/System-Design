...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, ডিস্ট্রিবিউটেড সিস্টেম রিট্রাই রিকোয়েস্টগুলো due to timeouts এবং transient ফেইলিউরগুলো।
...
এই জায়গায় `Idempotent Operations` সামনে আসে। সহজ ভাষায়, An idempotent operation can be safely retried multiple times without changing the final result beyond the first successful execution।
...
- Right: `Idempotent Operations` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Idempotent Operations` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any retriable write path (payments, booking, job submission, মেসেজ কনজিউমারগুলো)।
- business signal: ডিস্ট্রিবিউটেড সিস্টেম রিট্রাই রিকোয়েস্টগুলো due to timeouts এবং transient ফেইলিউরগুলো।
...
`Idempotent Operations` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: It হলো rarely optional জন্য distributed writes; do না skip it যদি রিট্রাইগুলো exist।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding রিট্রাইগুলো on writes সাথে no deduplication strategy।
...
- ধাপ ২: `Idempotent Operations` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Idempotent Operations` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: idempotency key, deduplication, safe retries, duplicate side effects, retry window।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Idempotent Operations` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: An idempotent operation can be safely retried multiple times without changing the final result beyond the first successful execution।
...
- business impact: ডিস্ট্রিবিউটেড সিস্টেম রিট্রাই রিকোয়েস্টগুলো due to timeouts এবং transient ফেইলিউরগুলো।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Idempotent Operations` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Idempotent Operations` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adding রিট্রাইগুলো on writes সাথে no deduplication strategy।
...
- `Idempotent Operations` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
