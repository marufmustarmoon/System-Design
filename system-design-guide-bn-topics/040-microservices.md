...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, They সাহায্য large টিমগুলো ship independently এবং scale parts of a সিস্টেম differently।
...
এই জায়গায় `Microservices` সামনে আসে। সহজ ভাষায়, Microservices split an application into smaller services, each owning a focused business capability।
...
- Right: `Microservices` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Microservices` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Large domains, টিম স্কেলিং, independently scalable components।
- business signal: They সাহায্য large টিমগুলো ship independently এবং scale parts of a সিস্টেম differently।
...
`Microservices` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Small টিমগুলো/products যেখানে a modular monolith হলো simpler এবং faster।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Splitting by CRUD tables এর বদলে business capabilities।
...
- ধাপ ২: `Microservices` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Microservices` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: service boundaries, data ownership, independent deployment, network calls, operational complexity।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Microservices` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Microservices split an application into smaller services, each owning a focused business capability।
...
- business impact: They সাহায্য large টিমগুলো ship independently এবং scale parts of a সিস্টেম differently।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Microservices` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Microservices` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Splitting by CRUD tables এর বদলে business capabilities।
...
- `Microservices` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
