...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Applications need durable স্টেট, indexing, querying, এবং concurrency control beyond simple files।
...
এই জায়গায় `Databases` সামনে আসে। সহজ ভাষায়, Databases are systems for storing, querying, and updating application data reliably।
...
- Right: `Databases` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Databases` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any সিস্টেম সাথে durable স্টেট এবং query needs।
- business signal: Applications need durable স্টেট, indexing, querying, এবং concurrency control beyond simple files।
...
`Databases` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না force one ডাটাবেজ to handle logs, graph traversal, এবং OLTP equally well।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Choosing a ডাটাবেজ by trend এর বদলে access pattern।
...
- ধাপ ২: `Databases` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Databases` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: data model, query pattern, indexing, consistency, scale trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Databases` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Databases are systems for storing, querying, and updating application data reliably।
...
- business impact: Applications need durable স্টেট, indexing, querying, এবং concurrency control beyond simple files।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Databases` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Databases` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Choosing a ডাটাবেজ by trend এর বদলে access pattern।
...
- `Databases` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
