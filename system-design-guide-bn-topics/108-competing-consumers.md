...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি increases থ্রুপুট এবং কমায় backlog by স্কেলিং কনজিউমারগুলো horizontally।
...
এই জায়গায় `Competing Consumers` সামনে আসে। সহজ ভাষায়, Competing consumers is a pattern where multiple workers consume from the same কিউ to process messages in parallel।
...
- Right: `Competing Consumers` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Competing Consumers` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Independent tasks সাথে high থ্রুপুট demand।
- business signal: এটি increases থ্রুপুট এবং কমায় backlog by স্কেলিং কনজিউমারগুলো horizontally।
...
`Competing Consumers` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Per-entity স্টেট transitions requiring strict ordering।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding more কনজিউমারগুলো to an ordered কিউ ছাড়া পার্টিশন strategy।
...
- ধাপ ২: `Competing Consumers` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Competing Consumers` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: delivery semantics, retry policy, idempotency, ordering, DLQ/back pressure।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Competing Consumers` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Competing consumers is a pattern where multiple workers consume from the same কিউ to process messages in parallel।
...
- business impact: এটি increases থ্রুপুট এবং কমায় backlog by স্কেলিং কনজিউমারগুলো horizontally।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Competing Consumers` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Competing Consumers` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adding more কনজিউমারগুলো to an ordered কিউ ছাড়া পার্টিশন strategy।
...
- `Competing Consumers` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
