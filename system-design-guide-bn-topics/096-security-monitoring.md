...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Prevention controls fail; detection হলো required to respond quickly to abuse এবং breaches।
...
এই জায়গায় `Security Monitoring` সামনে আসে। সহজ ভাষায়, Security monitoring tracks suspicious activity, policy violations, and attack signals across applications and infrastructure।
...
- Right: `Security Monitoring` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Security Monitoring` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any internet-facing সিস্টেম অথবা সিস্টেম handling sensitive ডেটা।
- business signal: Prevention controls fail; detection হলো required to respond quickly to abuse এবং breaches।
...
`Security Monitoring` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না treat WAF/রেট লিমিটিং as a replacement জন্য সিকিউরিটি মনিটরিং।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: কোনো অডিট লগিং জন্য auth/admin actions।
...
- ধাপ ২: `Security Monitoring` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Security Monitoring` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: metrics/logs/traces, SLI/SLO, alerts, dashboards, runbooks।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Security Monitoring` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Security monitoring tracks suspicious activity, policy violations, and attack signals across applications and infrastructure।
...
- business impact: Prevention controls fail; detection হলো required to respond quickly to abuse এবং breaches।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Security Monitoring` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Security Monitoring` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: কোনো অডিট লগিং জন্য auth/admin actions।
...
- `Security Monitoring` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
