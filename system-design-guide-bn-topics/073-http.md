...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি provides a standard, interoperable way জন্য ক্লায়েন্টগুলো এবং সার্ভারগুলো to exchange resources এবং metadata।
...
এই জায়গায় `HTTP` সামনে আসে। সহজ ভাষায়, HTTP is an application-layer protocol for request-response communication, widely used on the web and in APIs।
...
- Right: `HTTP` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`HTTP` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Public APIs, web/mobile backends, interoperable সার্ভিস interfaces।
- business signal: এটি provides a standard, interoperable way জন্য ক্লায়েন্টগুলো এবং সার্ভারগুলো to exchange resources এবং metadata।
...
`HTTP` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Ultra-low-ল্যাটেন্সি binary internal RPC paths যেখানে HTTP+JSON overhead হলো unnecessary।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Designing APIs ছাড়া status-code semantics, timeouts, অথবা idempotency considerations।
...
- ধাপ ২: `HTTP` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `HTTP` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: methods/status codes, headers, timeouts, cache-control, idempotency।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `HTTP` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: HTTP is an application-layer protocol for request-response communication, widely used on the web and in APIs।
...
- business impact: এটি provides a standard, interoperable way জন্য ক্লায়েন্টগুলো এবং সার্ভারগুলো to exchange resources এবং metadata।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `HTTP` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `HTTP` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Designing APIs ছাড়া status-code semantics, timeouts, অথবা idempotency considerations।
...
- `HTTP` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
