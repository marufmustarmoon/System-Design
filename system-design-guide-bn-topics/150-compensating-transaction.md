...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Multi-সার্ভিস workflows অনেক সময় পারে না ব্যবহার one ACID transaction জুড়ে all সিস্টেমগুলো।
...
এই জায়গায় `Compensating Transaction` সামনে আসে। সহজ ভাষায়, A compensating transaction is a workflow step that semantically undoes or offsets prior actions when part of a distributed process fails।
...
- Right: `Compensating Transaction` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Compensating Transaction` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Distributed workflows, sagas, multi-সার্ভিস business transactions।
- business signal: Multi-সার্ভিস workflows অনেক সময় পারে না ব্যবহার one ACID transaction জুড়ে all সিস্টেমগুলো।
...
`Compensating Transaction` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Single-ডাটাবেজ transactional operations যা পারে ব্যবহার normal ACID transactions directly।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Assuming distributed transactions জুড়ে all সার্ভিসগুলো হলো সবসময় practical।
...
- ধাপ ২: `Compensating Transaction` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Compensating Transaction` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: compensating, transaction, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Compensating Transaction` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: A compensating transaction is a workflow step that semantically undoes or offsets prior actions when part of a distributed process fails।
...
- business impact: Multi-সার্ভিস workflows অনেক সময় পারে না ব্যবহার one ACID transaction জুড়ে all সিস্টেমগুলো।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Compensating Transaction` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Compensating Transaction` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Assuming distributed transactions জুড়ে all সার্ভিসগুলো হলো সবসময় practical।
...
- `Compensating Transaction` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
