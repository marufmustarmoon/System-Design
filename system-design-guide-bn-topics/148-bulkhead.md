...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Recovery হলো hard যখন all threads, কিউগুলো, অথবা DB connections হলো saturated by one ফেইলিউর domain।
...
এই জায়গায় `Bulkhead` সামনে আসে। সহজ ভাষায়, In resiliency, bulkhead isolates resources so a failing workload or dependency cannot consume everything and block recovery।
...
- strongest signal: Shared সার্ভিসগুলো সাথে mixed-priority workloads এবং unstable dependencies।
- business signal: Recovery হলো hard যখন all threads, কিউগুলো, অথবা DB connections হলো saturated by one ফেইলিউর domain।
...
- wrong context: Very simple সার্ভিসগুলো যেখানে isolation খরচ হলো unnecessary।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Critical এবং batch workloads sharing the same worker pool এবং কিউ।
...
- সহজ সংজ্ঞা: In resiliency, bulkhead isolates resources so a failing workload or dependency cannot consume everything and block recovery।
...
- business impact: Recovery হলো hard যখন all threads, কিউগুলো, অথবা DB connections হলো saturated by one ফেইলিউর domain।
...
- red flag avoid করুন: Critical এবং batch workloads sharing the same worker pool এবং কিউ।
