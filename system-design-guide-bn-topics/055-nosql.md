...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি provides alternatives to relational সিস্টেমগুলো জন্য specific স্কেলিং, schema flexibility, এবং access-pattern needs।
...
এই জায়গায় `NoSQL` সামনে আসে। সহজ ভাষায়, NoSQL is a broad category of non-relational ডাটাবেজs, including key-value, document, wide-column, and graph stores।
...
- Right: `NoSQL` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`NoSQL` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Specific workload fit (key lookup, flexible docs, high-scale পার্টিশনগুলো, graph traversal)।
- business signal: এটি provides alternatives to relational সিস্টেমগুলো জন্য specific স্কেলিং, schema flexibility, এবং access-pattern needs।
...
`NoSQL` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: যখন relational constraints/joins/transactions হলো central এবং scale হলো manageable সাথে SQL।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "ব্যবহার NoSQL" ছাড়া naming the ডেটা model এবং কনসিসটেন্সি needs।
...
- ধাপ ২: `NoSQL` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `NoSQL` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: data model, query pattern, indexing, consistency, scale trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `NoSQL` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: NoSQL is a broad category of non-relational ডাটাবেজs, including key-value, document, wide-column, and graph stores।
...
- business impact: এটি provides alternatives to relational সিস্টেমগুলো জন্য specific স্কেলিং, schema flexibility, এবং access-pattern needs।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `NoSQL` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- `Uber, Netflix`-এর মতো সিস্টেমে একই pattern সব feature-এ একভাবে চলে না; context অনুযায়ী প্রয়োগ বদলায়।
- তাই `NoSQL` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "ব্যবহার NoSQL" ছাড়া naming the ডেটা model এবং কনসিসটেন্সি needs।
...
- `NoSQL` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
