...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Different features need different guarantees।
...
এই জায়গায় `Consistency Patterns` সামনে আসে। সহজ ভাষায়, Consistency patterns describe how quickly and how reliably updates become visible across a ডিস্ট্রিবিউটেড সিস্টেম।
...
- Right: `Consistency Patterns` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Consistency Patterns` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন classifying endpoints এবং ডেটা entities by correctness requirements।
- business signal: Different features need different guarantees।
...
`Consistency Patterns` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না discuss কনসিসটেন্সি as a generic slogan detached from product behavior।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: ব্যবহার করে "ইভেন্টুয়াল কনসিসটেন্সি" as an excuse জন্য undefined behavior।
...
- ধাপ ২: `Consistency Patterns` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Consistency Patterns` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: consistency guarantees, staleness window, read-after-write, eventual consistency, strong consistency।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Consistency Patterns` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Consistency patterns describe how quickly and how reliably updates become visible across a ডিস্ট্রিবিউটেড সিস্টেম।
...
- business impact: Different features need different guarantees।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Consistency Patterns` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Consistency Patterns` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: ব্যবহার করে "ইভেন্টুয়াল কনসিসটেন্সি" as an excuse জন্য undefined behavior।
...
- `Consistency Patterns` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
