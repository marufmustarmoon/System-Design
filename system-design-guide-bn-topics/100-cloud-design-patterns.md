...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, টিমগুলো repeatedly face the same issues (রিট্রাইগুলো, caching, failover, throttling, queueing, গেটওয়ে design)।
...
এই জায়গায় `Cloud Design Patterns` সামনে আসে। সহজ ভাষায়, Cloud design patterns are reusable solution patterns for common distributed-system problems in cloud environments (scaling, reliability, security, data, and integration)।
...
- Right: `Cloud Design Patterns` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Cloud Design Patterns` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: To explain a known problem/solution ট্রেড-অফ clearly এবং quickly।
- business signal: টিমগুলো repeatedly face the same issues (রিট্রাইগুলো, caching, failover, throttling, queueing, গেটওয়ে design)।
...
`Cloud Design Patterns` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না pattern-name-drop ছাড়া mapping it to requirements এবং ফেইলিউর মোড।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: ব্যবহার করে pattern names as substitutes জন্য design reasoning।
...
- ধাপ ২: `Cloud Design Patterns` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Cloud Design Patterns` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: problem fit, data flow, trade-off, failure case, migration/operations।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Cloud Design Patterns` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Cloud design patterns are reusable solution patterns for common distributed-system problems in cloud environments (scaling, reliability, security, data, and integration)।
...
- business impact: টিমগুলো repeatedly face the same issues (রিট্রাইগুলো, caching, failover, throttling, queueing, গেটওয়ে design)।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Cloud Design Patterns` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Cloud Design Patterns` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: ব্যবহার করে pattern names as substitutes জন্য design reasoning।
...
- `Cloud Design Patterns` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
