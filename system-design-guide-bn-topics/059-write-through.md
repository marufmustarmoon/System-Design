...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি keeps ক্যাশ এবং ডাটাবেজ more synchronized যখন/একইসাথে still serving future reads from ক্যাশ।
...
এই জায়গায় `Write-Through` সামনে আসে। সহজ ভাষায়, Write-through writes data to the ক্যাশ and the backing store in the same request path।
...
- Right: `Write-Through` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Write-Through` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Read-heavy ডেটা সাথে frequent read-পরে-write access।
- business signal: এটি keeps ক্যাশ এবং ডাটাবেজ more synchronized যখন/একইসাথে still serving future reads from ক্যাশ।
...
`Write-Through` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Write-heavy workloads সাথে many values rarely read again।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Assuming রাইট-থ্রু eliminates all staleness issues automatically।
...
- ধাপ ২: `Write-Through` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Write-Through` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: write path cache+DB, consistency, write latency, cache population, failure behavior।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Write-Through` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Write-through writes data to the ক্যাশ and the backing store in the same request path।
...
- business impact: এটি keeps ক্যাশ এবং ডাটাবেজ more synchronized যখন/একইসাথে still serving future reads from ক্যাশ।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Write-Through` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Write-Through` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Assuming রাইট-থ্রু eliminates all staleness issues automatically।
...
- `Write-Through` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
