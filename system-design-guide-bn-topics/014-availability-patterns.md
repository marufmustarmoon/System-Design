...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Real সিস্টেমগুলো fail all the time: process crashes, bad deploys, network splits, এবং hardware issues।
...
এই জায়গায় `Availability Patterns` সামনে আসে। সহজ ভাষায়, Availability patterns are common ways to keep a system serving requests when instances, hosts, or regions fail।
...
- Right: `Availability Patterns` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Availability Patterns` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন uptime/SLO হলো part of the requirement।
- business signal: Real সিস্টেমগুলো fail all the time: process crashes, bad deploys, network splits, এবং hardware issues।
...
`Availability Patterns` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না add multi-রিজিয়ন failover জন্য an internal tool সাথে low criticality।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "we add replicas" ছাড়া explaining ট্রাফিক switching এবং স্টেট handling।
...
- ধাপ ২: `Availability Patterns` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Availability Patterns` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: uptime target, downtime budget, failover, redundancy, dependency chain।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Availability Patterns` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Availability patterns are common ways to keep a system serving requests when instances, hosts, or regions fail।
...
- business impact: Real সিস্টেমগুলো fail all the time: process crashes, bad deploys, network splits, এবং hardware issues।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Availability Patterns` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Availability Patterns` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "we add replicas" ছাড়া explaining ট্রাফিক switching এবং স্টেট handling।
...
- `Availability Patterns` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
