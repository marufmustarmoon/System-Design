...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Some operations take too long জন্য normal synchronous রিকোয়েস্ট timeouts।
...
এই জায়গায় `Async Request Reply` সামনে আসে। সহজ ভাষায়, Async request-reply is a pattern where a client submits a request, gets an acknowledgment immediately, and retrieves the result later asynchronously।
...
- Right: `Async Request Reply` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Async Request Reply` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: লং-রানিং jobs, batch exports, media processing, external integrations।
- business signal: Some operations take too long জন্য normal synchronous রিকোয়েস্ট timeouts।
...
`Async Request Reply` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Short operations যেখানে sync রেসপন্স fits ল্যাটেন্সি budgets।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Async API সাথে no status endpoint, timeout policy, অথবা result retention design।
...
- ধাপ ২: `Async Request Reply` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Async Request Reply` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: delivery semantics, retry policy, idempotency, ordering, DLQ/back pressure।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Async Request Reply` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Async request-reply is a pattern where a client submits a request, gets an acknowledgment immediately, and retrieves the result later asynchronously।
...
- business impact: Some operations take too long জন্য normal synchronous রিকোয়েস্ট timeouts।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Async Request Reply` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Async Request Reply` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Async API সাথে no status endpoint, timeout policy, অথবা result retention design।
...
- `Async Request Reply` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
