...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Protocol এবং API style choices directly affect ল্যাটেন্সি, reliability, compatibility, এবং টিম velocity।
...
এই জায়গায় `Communication` সামনে আসে। সহজ ভাষায়, Communication patterns define how components exchange data and commands across a system।
...
- Right: `Communication` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Communication` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: In every design যেখানে components interact জুড়ে process/network boundaries।
- business signal: Protocol এবং API style choices directly affect ল্যাটেন্সি, reliability, compatibility, এবং টিম velocity।
...
`Communication` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না উপর-discuss protocol internals যদি the interview হলো focused on ডেটা modeling unless communication হলো the bottleneck।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Mixing protocol terms এবং API styles as যদি they হলো equivalent।
...
- ধাপ ২: `Communication` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Communication` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: communication, use case, trade-off, failure case, operations।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Communication` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Communication patterns define how components exchange data and commands across a system।
...
- business impact: Protocol এবং API style choices directly affect ল্যাটেন্সি, reliability, compatibility, এবং টিম velocity।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Communication` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Communication` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Mixing protocol terms এবং API styles as যদি they হলো equivalent।
...
- `Communication` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
