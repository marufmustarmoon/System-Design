...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Coordinator roles (scheduler, পার্টিশন owner, lock manager) হলো single-role functions যা need fast, safe recovery।
...
এই জায়গায় `Leader Election` সামনে আসে। সহজ ভাষায়, In resiliency, leader election ensures a failed coordinator can be replaced safely so distributed workflows continue।
...
- strongest signal: Coordinator সার্ভিসগুলো এবং singleton workers requiring continuity।
- business signal: Coordinator roles (scheduler, পার্টিশন owner, lock manager) হলো single-role functions যা need fast, safe recovery।
...
- wrong context: Stateless horizontally scaled সার্ভিসগুলো সাথে no singleton semantics।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: কোনো fencing/lease expiration in leader failover design।
...
- সহজ সংজ্ঞা: In resiliency, leader election ensures a failed coordinator can be replaced safely so distributed workflows continue।
...
- business impact: Coordinator roles (scheduler, পার্টিশন owner, lock manager) হলো single-role functions যা need fast, safe recovery।
...
- red flag avoid করুন: কোনো fencing/lease expiration in leader failover design।
