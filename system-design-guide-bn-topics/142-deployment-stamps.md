...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, HA সিস্টেমগুলো need isolation so one ডিপ্লয়মেন্ট অথবা infrastructure issue does না cause a global আউটেজ।
...
এই জায়গায় `Deployment Stamps` সামনে আসে। সহজ ভাষায়, In the HA context, deployment stamps are duplicated production units that limit blast radius and support fast traffic shift during failures।
...
- strongest signal: High-scale, high-SLO সিস্টেমগুলো needing blast-radius containment।
- business signal: HA সিস্টেমগুলো need isolation so one ডিপ্লয়মেন্ট অথবা infrastructure issue does না cause a global আউটেজ।
...
- wrong context: Single-টিম সিস্টেমগুলো ছাড়া capacity to operate many stamps।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: কোনো failover capacity planning জুড়ে stamps।
...
- সহজ সংজ্ঞা: In the HA context, deployment stamps are duplicated production units that limit blast radius and support fast traffic shift during failures।
...
- business impact: HA সিস্টেমগুলো need isolation so one ডিপ্লয়মেন্ট অথবা infrastructure issue does না cause a global আউটেজ।
...
- red flag avoid করুন: কোনো failover capacity planning জুড়ে stamps।
