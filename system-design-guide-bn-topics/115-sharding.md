...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি lets the ডেটা layer grow beyond one machine’s storage, IOPS, এবং write limits।
...
এই জায়গায় `Sharding` সামনে আসে। সহজ ভাষায়, As a data management pattern, শার্ডিং means partitioning large datasets across multiple storage nodes to scale capacity and থ্রুপুট।
...
- Right: `Sharding` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Sharding` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Sustained growth in write থ্রুপুট অথবা storage size beyond ভার্টিক্যাল স্কেলিং।
- business signal: এটি lets the ডেটা layer grow beyond one machine’s storage, IOPS, এবং write limits।
...
`Sharding` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Early সিস্টেমগুলো সাথে low volume এবং uncertain access patterns।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: শার্ডিং সাথে no plan জন্য cross-shard queries অথবা rebalancing।
...
- ধাপ ২: `Sharding` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Sharding` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: shard key, hot partition, rebalancing, cross-shard queries, data distribution।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Sharding` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: As a data management pattern, শার্ডিং means partitioning large datasets across multiple storage nodes to scale capacity and থ্রুপুট।
...
- business impact: এটি lets the ডেটা layer grow beyond one machine’s storage, IOPS, এবং write limits।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Sharding` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Sharding` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: শার্ডিং সাথে no plan জন্য cross-shard queries অথবা rebalancing।
...
- `Sharding` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
