...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি speeds repeated ডেটা access এবং কমায় disk I/O।
...
এই জায়গায় `Database Caching` সামনে আসে। সহজ ভাষায়, Database caching refers to caching at or near the ডাটাবেজ layer, including DB buffer/ক্যাশ pages and query result ক্যাশs (where supported)।
...
- Right: `Database Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Database Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: As a baseline optimization; understand what the DB already ক্যাশগুলো আগে adding more layers।
- business signal: এটি speeds repeated ডেটা access এবং কমায় disk I/O।
...
`Database Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না assume DB ক্যাশ পারে solve all hot-key অথবা network-ল্যাটেন্সি problems alone।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Ignoring built-in DB caching যখন diagnosing slow reads।
...
- ধাপ ২: `Database Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Database Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Database Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Database caching refers to caching at or near the ডাটাবেজ layer, including DB buffer/ক্যাশ pages and query result ক্যাশs (where supported)।
...
- business impact: এটি speeds repeated ডেটা access এবং কমায় disk I/O।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Database Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Database Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Ignoring built-in DB caching যখন diagnosing slow reads।
...
- `Database Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
