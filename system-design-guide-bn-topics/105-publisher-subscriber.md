...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি decouples প্রোডিউসারগুলো from many downstream কনজিউমারগুলো এবং সাপোর্ট করে fan-out ছাড়া প্রোডিউসার changes।
...
এই জায়গায় `Publisher / Subscriber` সামনে আসে। সহজ ভাষায়, Publisher/subscriber (pub/sub) is a messaging pattern where publishers emit events and multiple subscribers independently consume them।
...
- Right: `Publisher / Subscriber` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Publisher / Subscriber` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: ইভেন্ট-ড্রিভেন fan-out, analytics pipelines, loosely coupled integrations।
- business signal: এটি decouples প্রোডিউসারগুলো from many downstream কনজিউমারগুলো এবং সাপোর্ট করে fan-out ছাড়া প্রোডিউসার changes।
...
`Publisher / Subscriber` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Command processing যেখানে exactly one worker should execute the job।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: ব্যবহার করে pub/sub জন্য tightly coupled রিকোয়েস্ট-রেসপন্স workflows।
...
- ধাপ ২: `Publisher / Subscriber` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Publisher / Subscriber` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: delivery semantics, retry policy, idempotency, ordering, DLQ/back pressure।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Publisher / Subscriber` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Publisher/subscriber (pub/sub) is a messaging pattern where publishers emit events and multiple subscribers independently consume them।
...
- business impact: এটি decouples প্রোডিউসারগুলো from many downstream কনজিউমারগুলো এবং সাপোর্ট করে fan-out ছাড়া প্রোডিউসার changes।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Publisher / Subscriber` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Publisher / Subscriber` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: ব্যবহার করে pub/sub জন্য tightly coupled রিকোয়েস্ট-রেসপন্স workflows।
...
- `Publisher / Subscriber` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
