...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Different layers solve different ল্যাটেন্সি এবং লোড problems।
...
এই জায়গায় `Caching Strategies` সামনে আসে। সহজ ভাষায়, Caching strategies define where ক্যাশs live (client, CDN, server, app, DB) and how data moves through them।
...
- Right: `Caching Strategies` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Caching Strategies` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন a single ক্যাশ layer হলো না enough to meet ল্যাটেন্সি/খরচ goals।
- business signal: Different layers solve different ল্যাটেন্সি এবং লোড problems।
...
`Caching Strategies` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না stack multiple ক্যাশগুলো ছাড়া understanding যা one should own freshness behavior।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Proposing every ক্যাশ layer at once সাথে no clear reason।
...
- ধাপ ২: `Caching Strategies` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Caching Strategies` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Caching Strategies` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Caching strategies define where ক্যাশs live (client, CDN, server, app, DB) and how data moves through them।
...
- business impact: Different layers solve different ল্যাটেন্সি এবং লোড problems।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Caching Strategies` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Caching Strategies` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Proposing every ক্যাশ layer at once সাথে no clear reason।
...
- `Caching Strategies` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
