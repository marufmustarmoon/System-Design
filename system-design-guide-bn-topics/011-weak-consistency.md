...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, business target আর system limit একসাথে না ভাবলে bottleneck বারবার ফিরে আসে।
...
এই জায়গায় `Weak Consistency` সামনে আসে। সহজ ভাষায়, উইক কনসিসটেন্সি মানে there হলো no strong guarantee about যখন all ক্লায়েন্টগুলো will see the latest write।
...
- Right: `Weak Consistency` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
- প্রতিটি write-এর সাথে coordination দরকার হবে, তাই latency ও write contention বাড়বে।
- benefit: critical state (balance, inventory, booking)-এ correctness guarantee শক্তিশালী হয়।
- cost: throughput কমতে পারে এবং system scale করা কঠিন হয়।

### রাস্তা ২: Weak Consistency ধরলে

- async propagation ব্যবহার করলে response দ্রুত হয় এবং write path হালকা থাকে।
- downside: কিছু সময় stale read দেখা যাবে, তাই UI expectation ও refresh policy দরকার।
- non-critical signal/analytics/counting path-এ এটি প্র্যাকটিক্যাল trade-off।

### Weak Consistency আসলে কীভাবে সাহায্য করে?

`Weak Consistency` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: `Weak Consistency` ছাড়া একই সমস্যায় বারবার manual firefighting হচ্ছে।
- business signal: feature growth-এর সাথে reliability ও cost একসাথে সামলাতে হচ্ছে।
...
`Weak Consistency` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: requirement-fit না দেখে copy-paste architecture করা।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Calling a সিস্টেম weakly consistent ছাড়া naming ইউজার-visible impact।
...
- ধাপ ২: `Weak Consistency` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Weak Consistency` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: consistency, weak, may, signals, analytics।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Weak Consistency` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: উইক কনসিসটেন্সি মানে there হলো no strong guarantee about যখন all ক্লায়েন্টগুলো will see the latest write।
...
- business impact: ভুল design নিলে latency, outage risk, আর infra bill তিনটাই বাড়ে।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Weak Consistency` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Weak Consistency` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Calling a সিস্টেম weakly consistent ছাড়া naming ইউজার-visible impact।
...
- `Weak Consistency` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
