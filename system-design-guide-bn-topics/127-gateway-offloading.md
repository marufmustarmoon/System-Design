...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি কমায় duplicated implementation in backend সার্ভিসগুলো এবং standardizes behavior।
...
এই জায়গায় `Gateway Offloading` সামনে আসে। সহজ ভাষায়, Gateway offloading moves common cross-cutting work (TLS termination, auth, compression, rate limiting, caching) to the gateway layer।
...
- Right: `Gateway Offloading` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Gateway Offloading` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Repeated edge concerns জুড়ে many APIs।
- business signal: এটি কমায় duplicated implementation in backend সার্ভিসগুলো এবং standardizes behavior।
...
`Gateway Offloading` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: সার্ভিস-specific business validation যা requires domain context।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Treating গেটওয়ে as the place জন্য all business rules।
...
- ধাপ ২: `Gateway Offloading` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Gateway Offloading` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: routing policy, health checks, timeout/retry, failover, traffic distribution।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Gateway Offloading` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Gateway offloading moves common cross-cutting work (TLS termination, auth, compression, rate limiting, caching) to the gateway layer।
...
- business impact: এটি কমায় duplicated implementation in backend সার্ভিসগুলো এবং standardizes behavior।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Gateway Offloading` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Gateway Offloading` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Treating গেটওয়ে as the place জন্য all business rules।
...
- `Gateway Offloading` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
