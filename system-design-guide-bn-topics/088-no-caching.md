...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি causes avoidable ল্যাটেন্সি এবং লোড, especially এর অধীনে spikes।
...
এই জায়গায় `No Caching` সামনে আসে। সহজ ভাষায়, "No caching" as an antipattern means repeated expensive reads hit the source of truth every time, even when data is highly reusable।
...
- Right: `No Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`No Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: ব্যবহার এটি as a warning যখন hot reads dominate এবং ল্যাটেন্সি targets হলো tight।
- business signal: এটি causes avoidable ল্যাটেন্সি এবং লোড, especially এর অধীনে spikes।
...
`No Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না add ক্যাশ আগে proving repeated-read bottlenecks exist।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "DB পারে handle it" ছাড়া ট্রাফিক estimates জন্য hot keys।
...
- ধাপ ২: `No Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `No Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `No Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: "No caching" as an antipattern means repeated expensive reads hit the source of truth every time, even when data is highly reusable।
...
- business impact: এটি causes avoidable ল্যাটেন্সি এবং লোড, especially এর অধীনে spikes।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `No Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `No Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "DB পারে handle it" ছাড়া ট্রাফিক estimates জন্য hot keys।
...
- `No Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
