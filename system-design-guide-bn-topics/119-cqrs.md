...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Read এবং write workloads অনেক সময় have different পারফরম্যান্স, স্কেলিং, এবং ডেটা-shape needs।
...
এই জায়গায় `CQRS` সামনে আসে। সহজ ভাষায়, CQRS (Command Query Responsibility Segregation) separates write models (commands) from read models (queries), often with different schemas/stores।
...
- Right: `CQRS` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`CQRS` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Complex domains সাথে heavy reads এবং specialized query views।
- business signal: Read এবং write workloads অনেক সময় have different পারফরম্যান্স, স্কেলিং, এবং ডেটা-shape needs।
...
`CQRS` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Simple CRUD apps যেখানে one model/store হলো easier এবং sufficient।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Splitting read/write paths ছাড়া a clear পারফরম্যান্স অথবা domain reason।
...
- ধাপ ২: `CQRS` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `CQRS` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: read/write separation, projection models, eventual consistency, command vs query, projection rebuild।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `CQRS` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: CQRS (Command Query Responsibility Segregation) separates write models (commands) from read models (queries), often with different schemas/stores।
...
- business impact: Read এবং write workloads অনেক সময় have different পারফরম্যান্স, স্কেলিং, এবং ডেটা-shape needs।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `CQRS` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `CQRS` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Splitting read/write paths ছাড়া a clear পারফরম্যান্স অথবা domain reason।
...
- `CQRS` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
