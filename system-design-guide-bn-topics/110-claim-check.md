...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, মেসেজ brokers এবং APIs perform poorly সাথে very large payloads; large মেসেজগুলো এছাড়াও increase খরচ এবং রিট্রাই impact।
...
এই জায়গায় `Claim Check` সামনে আসে। সহজ ভাষায়, Claim check is a pattern where large payloads are stored externally (e.g., object storage) and messages carry only a reference/handle।
...
- Right: `Claim Check` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Claim Check` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Large files, large documents, media processing pipelines।
- business signal: মেসেজ brokers এবং APIs perform poorly সাথে very large payloads; large মেসেজগুলো এছাড়াও increase খরচ এবং রিট্রাই impact।
...
`Claim Check` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Small মেসেজগুলো যেখানে extra storage round-trip adds unnecessary complexity।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Sending huge blobs directly মাধ্যমে the কিউ/broker।
...
- ধাপ ২: `Claim Check` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Claim Check` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: claim, check, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Claim Check` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Claim check is a pattern where large payloads are stored externally (e.g., object storage) and messages carry only a reference/handle।
...
- business impact: মেসেজ brokers এবং APIs perform poorly সাথে very large payloads; large মেসেজগুলো এছাড়াও increase খরচ এবং রিট্রাই impact।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Claim Check` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Claim Check` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Sending huge blobs directly মাধ্যমে the কিউ/broker।
...
- `Claim Check` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
