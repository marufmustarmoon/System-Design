...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি সক্ষম করে dynamic config changes, consistent ডিপ্লয়মেন্ট settings, এবং separation of code from environment-specific values।
...
এই জায়গায় `External Config Store` সামনে আসে। সহজ ভাষায়, An external config store keeps configuration outside application binaries and hosts, usually in a centralized service।
...
- Right: `External Config Store` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`External Config Store` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Multi-সার্ভিস environments সাথে frequent config changes এবং staged rollouts।
- business signal: এটি সক্ষম করে dynamic config changes, consistent ডিপ্লয়মেন্ট settings, এবং separation of code from environment-specific values।
...
`External Config Store` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Tiny সিস্টেমগুলো সাথে static config এবং low operational overhead needs।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Runtime dependency on config store জন্য every রিকোয়েস্ট।
...
- ধাপ ২: `External Config Store` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `External Config Store` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: external, config, store, use case, trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `External Config Store` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: An external config store keeps configuration outside application binaries and hosts, usually in a centralized service।
...
- business impact: এটি সক্ষম করে dynamic config changes, consistent ডিপ্লয়মেন্ট settings, এবং separation of code from environment-specific values।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `External Config Store` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `External Config Store` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Runtime dependency on config store জন্য every রিকোয়েস্ট।
...
- `External Config Store` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
