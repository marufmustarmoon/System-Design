...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Critical ইউজার-facing সিস্টেমগুলো need very short outages to protect revenue, trust, অথবা safety।
...
এই জায়গায় `99.99% Availability (Four 9s)` সামনে আসে। সহজ ভাষায়, 99.99% অ্যাভেইলেবিলিটি means about 0.01% downtime is allowed।
...
- Right: `99.99% Availability (Four 9s)` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`99.99% Availability (Four 9s)` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Revenue-critical APIs, অথেন্টিকেশন, checkout, messaging delivery control plane।
- business signal: Critical ইউজার-facing সিস্টেমগুলো need very short outages to protect revenue, trust, অথবা safety।
...
`99.99% Availability (Four 9s)` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Low-value batch jobs যেখানে the খরচ of achieving four 9s outweighs business value।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Proposing four 9s যখন/একইসাথে keeping manual failover এবং no observability।
...
- ধাপ ২: `99.99% Availability (Four 9s)` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `99.99% Availability (Four 9s)` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: uptime target, downtime budget, failover, redundancy, dependency chain।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `99.99% Availability (Four 9s)` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: 99.99% অ্যাভেইলেবিলিটি means about 0.01% downtime is allowed।
...
- business impact: Critical ইউজার-facing সিস্টেমগুলো need very short outages to protect revenue, trust, অথবা safety।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `99.99% Availability (Four 9s)` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `99.99% Availability (Four 9s)` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Proposing four 9s যখন/একইসাথে keeping manual failover এবং no observability।
...
- `99.99% Availability (Four 9s)` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
