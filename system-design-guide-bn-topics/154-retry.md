...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Networks এবং সার্ভিসগুলো fail transiently; a later attempt অনেক সময় succeeds ছাড়া human intervention।
...
এই জায়গায় `Retry` সামনে আসে। সহজ ভাষায়, Retry is a pattern where failed operations are attempted again after a delay, usually for transient faults।
...
- Right: `Retry` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Retry` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Transient network ফেইলিউরগুলো, rate-limited APIs (সাথে backoff), কিউ processing।
- business signal: Networks এবং সার্ভিসগুলো fail transiently; a later attempt অনেক সময় succeeds ছাড়া human intervention।
...
`Retry` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Permanent errors (bad রিকোয়েস্টগুলো) অথবা non-আইডেমপোটেন্ট operations ছাড়া protection।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Immediate fixed-interval রিট্রাইগুলো জন্য all errors।
...
- ধাপ ২: `Retry` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Retry` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: exponential backoff, jitter, retry budget, transient failure, retry storm।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Retry` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Retry is a pattern where failed operations are attempted again after a delay, usually for transient faults।
...
- business impact: Networks এবং সার্ভিসগুলো fail transiently; a later attempt অনেক সময় succeeds ছাড়া human intervention।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Retry` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Retry` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Immediate fixed-interval রিট্রাইগুলো জন্য all errors।
...
- `Retry` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
