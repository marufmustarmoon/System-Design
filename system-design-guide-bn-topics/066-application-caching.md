...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি কমায় repeated ডাটাবেজ calls এবং expensive computations ব্যবহার করে application-specific logic।
...
এই জায়গায় `Application Caching` সামনে আসে। সহজ ভাষায়, Application caching stores computed results or data objects in the service layer (in-memory local ক্যাশ or distributed ক্যাশ)।
...
- Right: `Application Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Application Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Hot reads, expensive computations, rate-limited downstream calls।
- business signal: এটি কমায় repeated ডাটাবেজ calls এবং expensive computations ব্যবহার করে application-specific logic।
...
`Application Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Large object sets in memory-constrained সার্ভিসগুলো অথবা strict-freshness ডেটা ছাড়া invalidation সাপোর্ট।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: ব্যবহার করে local ক্যাশগুলো জুড়ে many instances এবং assuming consistent values।
...
- ধাপ ২: `Application Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Application Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Application Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Application caching stores computed results or data objects in the service layer (in-memory local ক্যাশ or distributed ক্যাশ)।
...
- business impact: এটি কমায় repeated ডাটাবেজ calls এবং expensive computations ব্যবহার করে application-specific logic।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Application Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Application Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: ব্যবহার করে local ক্যাশগুলো জুড়ে many instances এবং assuming consistent values।
...
- `Application Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
