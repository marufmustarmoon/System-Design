...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Persistent downstream ফেইলিউরগুলো পারে exhaust resources এবং block recovery যদি every রিকোয়েস্ট keeps attempting the dependency।
...
এই জায়গায় `Circuit Breaker` সামনে আসে। সহজ ভাষায়, In resiliency, circuit breakers help a service recover by failing fast against unhealthy dependencies and preventing repeated harm।
...
- Right: `Circuit Breaker` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Circuit Breaker` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Dependencies সাথে known transient/persistent ফেইলিউরগুলো এবং expensive timeouts।
- business signal: Persistent downstream ফেইলিউরগুলো পারে exhaust resources এবং block recovery যদি every রিকোয়েস্ট keeps attempting the dependency।
...
`Circuit Breaker` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না wrap everything blindly ছাড়া understanding fallback semantics।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Open circuit returns generic 500 সাথে no graceful degradation plan।
...
- ধাপ ২: `Circuit Breaker` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Circuit Breaker` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: failure threshold, open/half-open/closed, cascading failure, fallback, downstream protection।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Circuit Breaker` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: In resiliency, circuit breakers help a service recover by failing fast against unhealthy dependencies and preventing repeated harm।
...
- business impact: Persistent downstream ফেইলিউরগুলো পারে exhaust resources এবং block recovery যদি every রিকোয়েস্ট keeps attempting the dependency।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Circuit Breaker` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Circuit Breaker` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Open circuit returns generic 500 সাথে no graceful degradation plan।
...
- `Circuit Breaker` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
