...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি minimizes protocol overhead এবং ল্যাটেন্সি জন্য ব্যবহার cases যেখানে speed matters more than perfect delivery।
...
এই জায়গায় `UDP` সামনে আসে। সহজ ভাষায়, UDP is a connectionless transport protocol that sends datagrams without built-in reliability or ordering।
...
- Right: `UDP` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`UDP` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: রিয়েল-টাইম media, telemetry, DNS queries, custom low-ল্যাটেন্সি protocols।
- business signal: এটি minimizes protocol overhead এবং ল্যাটেন্সি জন্য ব্যবহার cases যেখানে speed matters more than perfect delivery।
...
`UDP` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Standard রিকোয়েস্ট-রেসপন্স business APIs needing reliable ordered delivery।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Choosing UDP জন্য a CRUD API just কারণ it হলো "faster."।
...
- ধাপ ২: `UDP` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `UDP` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: best-effort delivery, low overhead, packet loss tolerance, realtime traffic, application retries।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `UDP` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: UDP is a connectionless transport protocol that sends datagrams without built-in reliability or ordering।
...
- business impact: এটি minimizes protocol overhead এবং ল্যাটেন্সি জন্য ব্যবহার cases যেখানে speed matters more than perfect delivery।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `UDP` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `UDP` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Choosing UDP জন্য a CRUD API just কারণ it হলো "faster."।
...
- `UDP` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
