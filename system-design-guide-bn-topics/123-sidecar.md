...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি moves common infrastructure logic (proxying, telemetry, auth, config, TLS) out of app code।
...
এই জায়গায় `Sidecar` সামনে আসে। সহজ ভাষায়, A sidecar is a helper process/container deployed alongside an application instance to handle cross-cutting concerns।
...
- Right: `Sidecar` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Sidecar` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Cross-cutting networking/সিকিউরিটি/observability concerns জুড়ে many সার্ভিসগুলো।
- business signal: এটি moves common infrastructure logic (proxying, telemetry, auth, config, TLS) out of app code।
...
`Sidecar` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Small সিস্টেমগুলো যেখানে sidecar overhead এবং mesh complexity outweigh benefits।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding sidecars ছাড়া considering resource overhead এবং debugging impact।
...
- ধাপ ২: `Sidecar` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Sidecar` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: problem fit, data flow, trade-off, failure case, migration/operations।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Sidecar` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: A sidecar is a helper process/container deployed alongside an application instance to handle cross-cutting concerns।
...
- business impact: এটি moves common infrastructure logic (proxying, telemetry, auth, config, TLS) out of app code।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Sidecar` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- `Google, Uber`-এর মতো সিস্টেমে একই pattern সব feature-এ একভাবে চলে না; context অনুযায়ী প্রয়োগ বদলায়।
- তাই `Sidecar` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adding sidecars ছাড়া considering resource overhead এবং debugging impact।
...
- `Sidecar` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
