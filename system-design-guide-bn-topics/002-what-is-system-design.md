...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Code quality alone does না রোধ করতে outages caused by bottlenecks, contention, অথবা bad dependencies।
...
এই জায়গায় `What is System Design?` সামনে আসে। সহজ ভাষায়, It is the blueprint of how services, ডাটাবেজs, ক্যাশs, কিউs, and clients interact to solve a business problem।
...
- Right: `What is System Design?` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`What is System Design?` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন multiple components, টিমগুলো, অথবা reliability constraints হলো involved।
- business signal: Code quality alone does না রোধ করতে outages caused by bottlenecks, contention, অথবা bad dependencies।
...
`What is System Design?` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: যখন a coding interview asks জন্য an algorithm এবং আপনি উপর-architect the answer।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Defining সিস্টেম Design as শুধু "drawing boxes এবং arrows."।
...
- ধাপ ২: `What is System Design?` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `What is System Design?` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: requirements clarity, non-functional constraints, trade-offs, failure cases, structured answer।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `What is System Design?` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: It is the blueprint of how services, ডাটাবেজs, ক্যাশs, কিউs, and clients interact to solve a business problem।
...
- business impact: Code quality alone does না রোধ করতে outages caused by bottlenecks, contention, অথবা bad dependencies।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `What is System Design?` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `What is System Design?` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Defining সিস্টেম Design as শুধু "drawing boxes এবং arrows."।
...
- `What is System Design?` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
