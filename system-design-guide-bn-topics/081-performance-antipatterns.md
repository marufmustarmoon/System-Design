...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Interviewers want to know যদি আপনি পারে spot bottlenecks এবং poor architectural habits early।
...
এই জায়গায় `Performance Antipatterns` সামনে আসে। সহজ ভাষায়, Performance antipatterns are recurring design mistakes that make systems slow, fragile, or expensive under load।
...
- Right: `Performance Antipatterns` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Performance Antipatterns` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: সময় ট্রেড-অফ এবং bottleneck discussions to show operational maturity।
- business signal: Interviewers want to know যদি আপনি পারে spot bottlenecks এবং poor architectural habits early।
...
`Performance Antipatterns` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না recite a list ছাড়া tying issues to the proposed design।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Assuming the initial design has no bottlenecks কারণ it হলো "distributed."।
...
- ধাপ ২: `Performance Antipatterns` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Performance Antipatterns` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: problem fit, data flow, trade-off, failure case, migration/operations।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Performance Antipatterns` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Performance antipatterns are recurring design mistakes that make systems slow, fragile, or expensive under load।
...
- business impact: Interviewers want to know যদি আপনি পারে spot bottlenecks এবং poor architectural habits early।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Performance Antipatterns` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Performance Antipatterns` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Assuming the initial design has no bottlenecks কারণ it হলো "distributed."।
...
- `Performance Antipatterns` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
