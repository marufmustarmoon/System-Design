...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, In modern ডিপ্লয়মেন্টগুলো, instances come এবং go due to autoscaling, ফেইলিউরগুলো, এবং rolling ডিপ্লয়মেন্টগুলো।
...
এই জায়গায় `Service Discovery` সামনে আসে। সহজ ভাষায়, Service discovery is the mechanism services use to find the network location of other services dynamically।
...
- Right: `Service Discovery` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Service Discovery` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Dynamic infrastructure, microservices, container orchestration।
- business signal: In modern ডিপ্লয়মেন্টগুলো, instances come এবং go due to autoscaling, ফেইলিউরগুলো, এবং rolling ডিপ্লয়মেন্টগুলো।
...
`Service Discovery` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Small static ডিপ্লয়মেন্টগুলো সাথে fixed endpoints এবং low change frequency।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Hardcoding সার্ভিস IPs in a microservice design।
...
- ধাপ ২: `Service Discovery` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Service Discovery` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: service registry, instance health, dynamic endpoints, client/server-side discovery, registration।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Service Discovery` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Service discovery is the mechanism services use to find the network location of other services dynamically।
...
- business impact: In modern ডিপ্লয়মেন্টগুলো, instances come এবং go due to autoscaling, ফেইলিউরগুলো, এবং rolling ডিপ্লয়মেন্টগুলো।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Service Discovery` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Service Discovery` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Hardcoding সার্ভিস IPs in a microservice design।
...
- `Service Discovery` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
