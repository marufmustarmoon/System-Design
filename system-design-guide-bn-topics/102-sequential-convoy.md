...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Some workflows require strict ordering (যেমন, updates per ইউজার/order/account) এবং parallel কনজিউমারগুলো পারে break business rules।
...
এই জায়গায় `Sequential Convoy` সামনে আসে। সহজ ভাষায়, Sequential convoy is a pattern where work that must stay ordered is processed in sequence, often by partition/key, to preserve correctness।
...
- Right: `Sequential Convoy` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Sequential Convoy` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Per-entity ordered workflows এবং স্টেট machines।
- business signal: Some workflows require strict ordering (যেমন, updates per ইউজার/order/account) এবং parallel কনজিউমারগুলো পারে break business rules।
...
`Sequential Convoy` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Independent tasks যা পারে হতে processed in parallel ছাড়া ordering constraints।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Requiring global ordering যখন শুধু per-key ordering হলো needed।
...
- ধাপ ২: `Sequential Convoy` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Sequential Convoy` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: sequential, convoy, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Sequential Convoy` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Sequential convoy is a pattern where work that must stay ordered is processed in sequence, often by partition/key, to preserve correctness।
...
- business impact: Some workflows require strict ordering (যেমন, updates per ইউজার/order/account) এবং parallel কনজিউমারগুলো পারে break business rules।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Sequential Convoy` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Sequential Convoy` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Requiring global ordering যখন শুধু per-key ordering হলো needed।
...
- `Sequential Convoy` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
