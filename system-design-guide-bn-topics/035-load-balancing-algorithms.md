...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Different workloads এবং backend behaviors need different distribution strategies।
...
এই জায়গায় `Load Balancing Algorithms` সামনে আসে। সহজ ভাষায়, These are rules used by a লোড ব্যালেন্সার to choose which backend instance receives each request।
...
- Right: `Load Balancing Algorithms` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Load Balancing Algorithms` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন ট্রাফিক patterns vary অথবা backends হলো না identical।
- business signal: Different workloads এবং backend behaviors need different distribution strategies।
...
`Load Balancing Algorithms` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না উপর-optimize algorithm selection আগে confirming backend bottlenecks।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Picking an algorithm ছাড়া considering connection duration অথবা রিকোয়েস্ট খরচ skew।
...
- ধাপ ২: `Load Balancing Algorithms` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Load Balancing Algorithms` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: round robin, least connections, weighted routing, session affinity, skew/hotspots।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Load Balancing Algorithms` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: These are rules used by a লোড ব্যালেন্সার to choose which backend instance receives each request।
...
- business impact: Different workloads এবং backend behaviors need different distribution strategies।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Load Balancing Algorithms` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Load Balancing Algorithms` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Picking an algorithm ছাড়া considering connection duration অথবা রিকোয়েস্ট খরচ skew।
...
- `Load Balancing Algorithms` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
