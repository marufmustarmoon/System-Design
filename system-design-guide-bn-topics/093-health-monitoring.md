...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি drives লোড balancer routing, autoscaling decisions, এবং fast incident detection।
...
এই জায়গায় `Health Monitoring` সামনে আসে। সহজ ভাষায়, Health monitoring checks whether components are alive and able to serve traffic correctly।
...
- Right: `Health Monitoring` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Health Monitoring` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any সার্ভিস behind a লোড balancer অথবা orchestrator।
- business signal: এটি drives লোড balancer routing, autoscaling decisions, এবং fast incident detection।
...
`Health Monitoring` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না make হেলথ এন্ডপয়েন্টs depend on every downstream সার্ভিস unless আপনি want cascading removals।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: ব্যবহার করে one হেলথ এন্ডপয়েন্ট জন্য both liveness এবং readiness সাথে identical logic।
...
- ধাপ ২: `Health Monitoring` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Health Monitoring` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: metrics/logs/traces, SLI/SLO, alerts, dashboards, runbooks।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Health Monitoring` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Health monitoring checks whether components are alive and able to serve traffic correctly।
...
- business impact: এটি drives লোড balancer routing, autoscaling decisions, এবং fast incident detection।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Health Monitoring` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Health Monitoring` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: ব্যবহার করে one হেলথ এন্ডপয়েন্ট জন্য both liveness এবং readiness সাথে identical logic।
...
- `Health Monitoring` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
