...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, ডিস্ট্রিবিউটেড সিস্টেম face network ফেইলিউরগুলো এবং replica lag।
...
এই জায়গায় `Availability vs Consistency` সামনে আসে। সহজ ভাষায়, Availability means the system responds to requests (even if data may be stale)।
...
- Right: `Availability vs Consistency` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Availability vs Consistency` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন designing replicated ডেটা stores, failover behavior, অথবা multi-রিজিয়ন reads।
- business signal: ডিস্ট্রিবিউটেড সিস্টেম face network ফেইলিউরগুলো এবং replica lag।
...
`Availability vs Consistency` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না force স্ট্রং কনসিসটেন্সি everywhere যদি it kills ল্যাটেন্সি এবং অ্যাভেইলেবিলিটি ছাড়া business need।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "কনসিসটেন্সি" ছাড়া specifying what ডেটা এবং what guarantee।
...
- ধাপ ২: `Availability vs Consistency` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Availability vs Consistency` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: consistency guarantee, staleness window, read/write behavior, user-visible anomalies, trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Availability vs Consistency` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Availability means the system responds to requests (even if data may be stale)।
...
- business impact: ডিস্ট্রিবিউটেড সিস্টেম face network ফেইলিউরগুলো এবং replica lag।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Availability vs Consistency` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Availability vs Consistency` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "কনসিসটেন্সি" ছাড়া specifying what ডেটা এবং what guarantee।
...
- `Availability vs Consistency` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
