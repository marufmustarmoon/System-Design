...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি রোধ করে resource exhaustion এবং রিট্রাই স্টর্মs যখন/একইসাথে a dependency হলো unhealthy।
...
এই জায়গায় `Circuit Breaker` সামনে আসে। সহজ ভাষায়, Circuit breaker is a pattern that stops calling a failing/slow dependency for a period and fails fast instead।
...
- strongest signal: Unreliable অথবা variable-ল্যাটেন্সি dependencies, fan-out রিকোয়েস্ট paths।
- business signal: এটি রোধ করে resource exhaustion এবং রিট্রাই স্টর্মs যখন/একইসাথে a dependency হলো unhealthy।
...
- wrong context: করবেন না ব্যবহার it as a substitute জন্য fixing a permanently failing dependency।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: সার্কিট ব্রেকার সাথে রিট্রাইগুলো যা still hammer the dependency।
...
- সহজ সংজ্ঞা: Circuit breaker is a pattern that stops calling a failing/slow dependency for a period and fails fast instead।
...
- business impact: এটি রোধ করে resource exhaustion এবং রিট্রাই স্টর্মs যখন/একইসাথে a dependency হলো unhealthy।
...
- red flag avoid করুন: সার্কিট ব্রেকার সাথে রিট্রাইগুলো যা still hammer the dependency।
