...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Shared pools (threads, connections, কিউগুলো) পারে let one dependency ফেইলিউর spread everywhere।
...
এই জায়গায় `Bulkhead` সামনে আসে। সহজ ভাষায়, Bulkhead is a pattern that isolates resources so failure or overload in one part of the system does not sink the whole system।
...
- Right: `Bulkhead` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Bulkhead` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Multi-dependency সার্ভিসগুলো এবং shared platforms সাথে mixed-criticality workloads।
- business signal: Shared pools (threads, connections, কিউগুলো) পারে let one dependency ফেইলিউর spread everywhere।
...
`Bulkhead` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Tiny সার্ভিসগুলো সাথে one dependency এবং low risk, যেখানে extra partitioning adds little value।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: One shared thread/connection pool জন্য all dependencies।
...
- ধাপ ২: `Bulkhead` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Bulkhead` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: failure isolation, resource pools, blast radius, tenant isolation, capacity partitioning।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Bulkhead` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Bulkhead is a pattern that isolates resources so failure or overload in one part of the system does not sink the whole system।
...
- business impact: Shared pools (threads, connections, কিউগুলো) পারে let one dependency ফেইলিউর spread everywhere।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Bulkhead` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Bulkhead` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: One shared thread/connection pool জন্য all dependencies।
...
- `Bulkhead` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
