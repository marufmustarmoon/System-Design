...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Automated failover এবং ট্রাফিক removal require timely, machine-readable health signals।
...
এই জায়গায় `Health Endpoint Monitoring` সামনে আসে। সহজ ভাষায়, Health endpoint monitoring is a reliability pattern that continuously probes service health endpoints to make routing and recovery decisions।
...
- Right: `Health Endpoint Monitoring` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Health Endpoint Monitoring` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any autoscaled অথবা লোড-balanced সার্ভিস requiring automated ট্রাফিক management।
- business signal: Automated failover এবং ট্রাফিক removal require timely, machine-readable health signals।
...
`Health Endpoint Monitoring` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না wire হেলথ চেক to flaky dependencies in a way যা removes healthy instances unnecessarily।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Single হেলথ এন্ডপয়েন্ট যা সবসময় returns success regardless of readiness।
...
- ধাপ ২: `Health Endpoint Monitoring` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Health Endpoint Monitoring` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: metrics/logs/traces, SLI/SLO, alerts, dashboards, runbooks।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Health Endpoint Monitoring` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Health endpoint monitoring is a reliability pattern that continuously probes service health endpoints to make routing and recovery decisions।
...
- business impact: Automated failover এবং ট্রাফিক removal require timely, machine-readable health signals।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Health Endpoint Monitoring` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Health Endpoint Monitoring` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Single হেলথ এন্ডপয়েন্ট যা সবসময় returns success regardless of readiness।
...
- `Health Endpoint Monitoring` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
