...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি decouples প্রোডিউসারগুলো এবং কনজিউমারগুলো এবং অনুমতি দেয় multiple downstream actions ছাড়া tightly coupled synchronous calls।
...
এই জায়গায় `Event-Driven` সামনে আসে। সহজ ভাষায়, Event-driven processing triggers work when a specific event occurs (e.g., order placed, file uploaded)।
...
- Right: `Event-Driven` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Event-Driven` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Reactive workflows, fan-out side effects, অডিট trails।
- business signal: এটি decouples প্রোডিউসারগুলো এবং কনজিউমারগুলো এবং অনুমতি দেয় multiple downstream actions ছাড়া tightly coupled synchronous calls।
...
`Event-Driven` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Simple sequential workflows যেখানে synchronous calls হলো clearer এবং easier to debug।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Treating ইভেন্টগুলো as reliable commands ছাড়া আইডেমপোটেন্ট কনজিউমারগুলো।
...
- ধাপ ২: `Event-Driven` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Event-Driven` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: domain events, publish/subscribe, idempotent consumers, schema evolution, event tracing।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Event-Driven` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Event-driven processing triggers work when a specific event occurs (e.g., order placed, file uploaded)।
...
- business impact: এটি decouples প্রোডিউসারগুলো এবং কনজিউমারগুলো এবং অনুমতি দেয় multiple downstream actions ছাড়া tightly coupled synchronous calls।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Event-Driven` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Event-Driven` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Treating ইভেন্টগুলো as reliable commands ছাড়া আইডেমপোটেন্ট কনজিউমারগুলো।
...
- `Event-Driven` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
