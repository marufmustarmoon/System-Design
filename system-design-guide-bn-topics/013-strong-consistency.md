...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি simplifies reasoning about correctness জন্য critical operations।
...
এই জায়গায় `Strong Consistency` সামনে আসে। সহজ ভাষায়, Strong কনসিসটেন্সি means reads reflect the latest successful write according to the system’s chosen ordering guarantees।
...
- Right: `Strong Consistency` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Strong Consistency` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Money movement, unique constraints, transactional workflows, lock সার্ভিসগুলো।
- business signal: এটি simplifies reasoning about correctness জন্য critical operations।
...
`Strong Consistency` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: High-volume derived ডেটা যেখানে small staleness হলো acceptable।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Requiring স্ট্রং কনসিসটেন্সি everywhere ছাড়া discussing ল্যাটেন্সি/SLO impact।
...
- ধাপ ২: `Strong Consistency` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Strong Consistency` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: latest read guarantee, coordination, quorum/leader reads, latency trade-off, critical workflows।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Strong Consistency` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Strong কনসিসটেন্সি means reads reflect the latest successful write according to the system’s chosen ordering guarantees।
...
- business impact: এটি simplifies reasoning about correctness জন্য critical operations।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Strong Consistency` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Strong Consistency` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Requiring স্ট্রং কনসিসটেন্সি everywhere ছাড়া discussing ল্যাটেন্সি/SLO impact।
...
- `Strong Consistency` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
