...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Interview prompts হলো intentionally open-ended।
...
এই জায়গায় `How to approach System Design?` সামনে আসে। সহজ ভাষায়, A repeatable interview process for turning a vague prompt into a concrete, defensible design।
...
- Right: `How to approach System Design?` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`How to approach System Design?` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: In every সিস্টেম Design interview, especially ambiguous prompts।
- business signal: Interview prompts হলো intentionally open-ended।
...
`How to approach System Design?` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: আপনি should না rigidly follow a checklist যদি the interviewer asks to deep-dive a specific area।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Spending 15 minutes estimating QPS আগে understanding product behavior।
...
- ধাপ ২: `How to approach System Design?` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `How to approach System Design?` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: how, requirements clarity, non-functional constraints, trade-offs, failure cases।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `How to approach System Design?` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: A repeatable interview process for turning a vague prompt into a concrete, defensible design।
...
- business impact: Interview prompts হলো intentionally open-ended।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `How to approach System Design?` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `How to approach System Design?` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Spending 15 minutes estimating QPS আগে understanding product behavior।
...
- `How to approach System Design?` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
