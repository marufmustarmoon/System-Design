...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Primary ডেটা models অনেক সময় optimize one access path; সিস্টেমগুলো need fast alternate lookups ছাড়া full scans।
...
এই জায়গায় `Index Table` সামনে আসে। সহজ ভাষায়, An index table is an additional table/store optimized for a specific lookup path (for example, lookup by email to user ID)।
...
- Right: `Index Table` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Index Table` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Alternate key lookups in NoSQL stores অথবা read models।
- business signal: Primary ডেটা models অনেক সময় optimize one access path; সিস্টেমগুলো need fast alternate lookups ছাড়া full scans।
...
`Index Table` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Simple relational workloads যেখানে native ইনডেক্সগুলো হলো sufficient।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding ইনডেক্স tables ছাড়া describing write/update flow।
...
- ধাপ ২: `Index Table` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Index Table` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: index, table, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Index Table` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: An index table is an additional table/store optimized for a specific lookup path (for example, lookup by email to user ID)।
...
- business impact: Primary ডেটা models অনেক সময় optimize one access path; সিস্টেমগুলো need fast alternate lookups ছাড়া full scans।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Index Table` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Index Table` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adding ইনডেক্স tables ছাড়া describing write/update flow।
...
- `Index Table` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
