...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি সাহায্য করে frontend/mobile ক্লায়েন্টগুলো এড়াতে উপর-fetching এবং এর অধীনে-fetching জুড়ে multiple resources।
...
এই জায়গায় `GraphQL` সামনে আসে। সহজ ভাষায়, GraphQL is a query language and API runtime where clients request exactly the fields they need from a typed schema।
...
- Right: `GraphQL` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`GraphQL` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: UI aggregation, many ক্লায়েন্ট variants, evolving frontend ডেটা needs।
- business signal: এটি সাহায্য করে frontend/mobile ক্লায়েন্টগুলো এড়াতে উপর-fetching এবং এর অধীনে-fetching জুড়ে multiple resources।
...
`GraphQL` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Simple APIs, internal সার্ভিস calls, অথবা workloads যেখানে query খরচ control হলো hard।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adopting GraphQL ছাড়া query depth/খরচ limits এবং resolver batching।
...
- ধাপ ২: `GraphQL` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `GraphQL` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: schema/resolvers, N+1 queries, query complexity, field-level auth, over/under-fetching।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `GraphQL` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: GraphQL is a query language and API runtime where clients request exactly the fields they need from a typed schema।
...
- business impact: এটি সাহায্য করে frontend/mobile ক্লায়েন্টগুলো এড়াতে উপর-fetching এবং এর অধীনে-fetching জুড়ে multiple resources।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `GraphQL` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `GraphQL` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Adopting GraphQL ছাড়া query depth/খরচ limits এবং resolver batching।
...
- `GraphQL` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
