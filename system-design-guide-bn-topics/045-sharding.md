...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি হলো a primary way to scale write থ্রুপুট এবং storage beyond a single DB node।
...
এই জায়গায় `Sharding` সামনে আসে। সহজ ভাষায়, Sharding splits data across multiple ডাটাবেজ partitions (shards), each storing a subset of the data।
...
- strongest signal: Large datasets, write-heavy workloads, clear পার্টিশন key access patterns।
- business signal: এটি হলো a primary way to scale write থ্রুপুট এবং storage beyond a single DB node।
...
- wrong context: Small সিস্টেমগুলো অথবা workloads requiring many cross-shard joins/transactions।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Picking a shard key ছাড়া discussing skew এবং rebalancing।
...
- সহজ সংজ্ঞা: Sharding splits data across multiple ডাটাবেজ partitions (shards), each storing a subset of the data।
...
- business impact: এটি হলো a primary way to scale write থ্রুপুট এবং storage beyond a single DB node।
...
- red flag avoid করুন: Picking a shard key ছাড়া discussing skew এবং rebalancing।
