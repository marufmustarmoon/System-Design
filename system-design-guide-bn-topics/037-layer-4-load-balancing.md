...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি offers lower overhead এবং কাজ করে জন্য many protocols beyond HTTP।
...
এই জায়গায় `Layer 4 Load Balancing` সামনে আসে। সহজ ভাষায়, Layer 4 (transport-layer) লোড ব্যালেন্সিং routes traffic using IP/port and transport-level information without HTTP awareness।
...
- Right: `Layer 4 Load Balancing` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Layer 4 Load Balancing` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Generic TCP/UDP সার্ভিসগুলো, very high-থ্রুপুট ট্রাফিক, simpler routing needs।
- business signal: এটি offers lower overhead এবং কাজ করে জন্য many protocols beyond HTTP।
...
`Layer 4 Load Balancing` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: HTTP APIs requiring host/path routing, auth checks, অথবা header-based decisions at the edge।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Suggesting L4 জন্য URL path routing।
...
- ধাপ ২: `Layer 4 Load Balancing` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Layer 4 Load Balancing` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: TCP/UDP forwarding, low overhead, connection-level balancing, health checks, L4 vs L7 trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Layer 4 Load Balancing` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Layer 4 (transport-layer) লোড ব্যালেন্সিং routes traffic using IP/port and transport-level information without HTTP awareness।
...
- business impact: এটি offers lower overhead এবং কাজ করে জন্য many protocols beyond HTTP।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Layer 4 Load Balancing` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Layer 4 Load Balancing` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Suggesting L4 জন্য URL path routing।
...
- `Layer 4 Load Balancing` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
