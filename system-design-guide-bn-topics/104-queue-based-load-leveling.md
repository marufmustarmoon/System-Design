...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, প্রোডিউসারগুলো এবং কনজিউমারগুলো rarely run at the same speed।
...
এই জায়গায় `Queue-Based Load Leveling` সামনে আসে। সহজ ভাষায়, Queue-based load leveling inserts a কিউ between producers and consumers so incoming bursts are buffered and processed at a steady rate।
...
- strongest signal: Burst absorption, async processing, rate-limited downstream dependencies।
- business signal: প্রোডিউসারগুলো এবং কনজিউমারগুলো rarely run at the same speed।
...
- wrong context: ইউজার paths requiring immediate synchronous completion।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Adding a কিউ ছাড়া worker স্কেলিং, DLQ, অথবা রিট্রাই policy।
...
- সহজ সংজ্ঞা: Queue-based load leveling inserts a কিউ between producers and consumers so incoming bursts are buffered and processed at a steady rate।
...
- business impact: প্রোডিউসারগুলো এবং কনজিউমারগুলো rarely run at the same speed।
...
- red flag avoid করুন: Adding a কিউ ছাড়া worker স্কেলিং, DLQ, অথবা রিট্রাই policy।
