...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি কমায় central workflow coupling এবং অনুমতি দেয় সার্ভিসগুলো to evolve independently।
...
এই জায়গায় `Choreography` সামনে আসে। সহজ ভাষায়, Choreography is a workflow style where services react to events and coordinate indirectly, without a central orchestrator।
...
- Right: `Choreography` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Choreography` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Loosely coupled ইভেন্ট-ড্রিভেন workflows সাথে independent domain সার্ভিসগুলো।
- business signal: এটি কমায় central workflow coupling এবং অনুমতি দেয় সার্ভিসগুলো to evolve independently।
...
`Choreography` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Complex workflows requiring centralized deadlines, compensations, অথবা strict sequencing।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Choreography সাথে no ইভেন্ট tracing/correlation IDs।
...
- ধাপ ২: `Choreography` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Choreography` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: data model, query pattern, indexing, consistency, scale trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Choreography` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Choreography is a workflow style where services react to events and coordinate indirectly, without a central orchestrator।
...
- business impact: এটি কমায় central workflow coupling এবং অনুমতি দেয় সার্ভিসগুলো to evolve independently।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Choreography` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Choreography` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Choreography সাথে no ইভেন্ট tracing/correlation IDs।
...
- `Choreography` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
