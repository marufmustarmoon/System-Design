...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, সিস্টেমগুলো অনেক সময় need one more than the other: ইউজার-facing APIs care about ল্যাটেন্সি; batch pipelines care about থ্রুপুট।
...
এই জায়গায় `Latency vs Throughput` সামনে আসে। সহজ ভাষায়, Latency is the time for one request/task to complete।
...
- Right: `Latency vs Throughput` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Latency vs Throughput` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন choosing sync vs async processing, batch size, অথবা queueing strategy।
- business signal: সিস্টেমগুলো অনেক সময় need one more than the other: ইউজার-facing APIs care about ল্যাটেন্সি; batch pipelines care about থ্রুপুট।
...
`Latency vs Throughput` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না treat average ল্যাটেন্সি alone as sufficient জন্য SLO discussion।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Ignoring p99 ল্যাটেন্সি on ইউজার-facing features।
...
- ধাপ ২: `Latency vs Throughput` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Latency vs Throughput` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: bottleneck, capacity planning, latency/throughput, scaling strategy, cost trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Latency vs Throughput` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Latency is the time for one request/task to complete।
...
- business impact: সিস্টেমগুলো অনেক সময় need one more than the other: ইউজার-facing APIs care about ল্যাটেন্সি; batch pipelines care about থ্রুপুট।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Latency vs Throughput` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Latency vs Throughput` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Ignoring p99 ল্যাটেন্সি on ইউজার-facing features।
...
- `Latency vs Throughput` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
