...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি হলো simpler to operate কারণ content হলো fetched on demand ছাড়া a separate publish step।
...
এই জায়গায় `Pull CDNs` সামনে আসে। সহজ ভাষায়, In a pull CDN model, the CDN fetches content from the origin on ক্যাশ miss and then ক্যাশs it for future requests।
...
- Right: `Pull CDNs` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Pull CDNs` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Web assets, APIs সাথে ক্যাশ headers, simpler CDN rollouts।
- business signal: এটি হলো simpler to operate কারণ content হলো fetched on demand ছাড়া a separate publish step।
...
`Pull CDNs` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Time-critical launches যেখানে ক্যাশ misses would overload the origin।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Ignoring origin shielding/rate limits যখন ব্যবহার করে a pull CDN।
...
- ধাপ ২: `Pull CDNs` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Pull CDNs` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache miss, origin fetch, cache hit ratio, cold content, origin protection।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Pull CDNs` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: In a pull CDN model, the CDN fetches content from the origin on ক্যাশ miss and then ক্যাশs it for future requests।
...
- business impact: এটি হলো simpler to operate কারণ content হলো fetched on demand ছাড়া a separate publish step।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Pull CDNs` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Pull CDNs` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Ignoring origin shielding/rate limits যখন ব্যবহার করে a pull CDN।
...
- `Pull CDNs` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
