...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, In ডিস্ট্রিবিউটেড সিস্টেম, ফেইলিউরগুলো হলো unavoidable; resilient সিস্টেমগুলো fail partially, recover quickly, এবং এড়াতে cascading outages।
...
এই জায়গায় `Resiliency` সামনে আসে। সহজ ভাষায়, Resiliency is the ability of a system to absorb faults, recover, and continue delivering acceptable behavior।
...
- Right: `Resiliency` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Resiliency` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Any production সিস্টেম সাথে dependencies, async workflows, এবং external integrations।
- business signal: In ডিস্ট্রিবিউটেড সিস্টেম, ফেইলিউরগুলো হলো unavoidable; resilient সিস্টেমগুলো fail partially, recover quickly, এবং এড়াতে cascading outages।
...
`Resiliency` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না overbuild resiliency controls জন্য trivial offline tools।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: শুধু discussing redundancy, না recovery behavior।
...
- ধাপ ২: `Resiliency` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Resiliency` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: failure handling, backoff/jitter, fallback, isolation, cascading failure।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Resiliency` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Resiliency is the ability of a system to absorb faults, recover, and continue delivering acceptable behavior।
...
- business impact: In ডিস্ট্রিবিউটেড সিস্টেম, ফেইলিউরগুলো হলো unavoidable; resilient সিস্টেমগুলো fail partially, recover quickly, এবং এড়াতে cascading outages।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Resiliency` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Resiliency` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: শুধু discussing redundancy, না recovery behavior।
...
- `Resiliency` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
