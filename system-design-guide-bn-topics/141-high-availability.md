...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Revenue-critical এবং mission-critical সিস্টেমগুলো পারে না tolerate long অথবা frequent outages।
...
এই জায়গায় `High Availability` সামনে আসে। সহজ ভাষায়, High অ্যাভেইলেবিলিটি is the discipline of designing systems to meet very high uptime targets (often measured in nines) with minimal service interruption।
...
- Right: `High Availability` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`High Availability` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Critical paths সাথে strict SLO/SLA targets।
- business signal: Revenue-critical এবং mission-critical সিস্টেমগুলো পারে না tolerate long অথবা frequent outages।
...
`High Availability` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Low-value internal tools যেখানে HA খরচ/complexity হলো unjustified।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Claiming HA সাথে single-রিজিয়ন DB primary এবং manual failover।
...
- ধাপ ২: `High Availability` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `High Availability` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: uptime target, downtime budget, failover, redundancy, dependency chain।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `High Availability` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: High অ্যাভেইলেবিলিটি is the discipline of designing systems to meet very high uptime targets (often measured in nines) with minimal service interruption।
...
- business impact: Revenue-critical এবং mission-critical সিস্টেমগুলো পারে না tolerate long অথবা frequent outages।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `High Availability` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `High Availability` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Claiming HA সাথে single-রিজিয়ন DB primary এবং manual failover।
...
- `High Availability` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
