...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি উন্নত করে capacity এবং resilience beyond vertical hardware limits।
...
এই জায়গায় `Horizontal Scaling` সামনে আসে। সহজ ভাষায়, Horizontal scaling means adding more instances/nodes instead of making one machine bigger।
...
- Right: `Horizontal Scaling` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Horizontal Scaling` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: ট্রাফিক growth, HA requirements, elastic workloads।
- business signal: এটি উন্নত করে capacity এবং resilience beyond vertical hardware limits।
...
`Horizontal Scaling` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Tiny সার্ভিসগুলো যেখানে orchestration overhead exceeds benefit।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Saying "just add more সার্ভারগুলো" যখন/একইসাথে keeping local session/স্টেট on each সার্ভার।
...
- ধাপ ২: `Horizontal Scaling` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Horizontal Scaling` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: stateless services, load balancer, shared dependency bottleneck, autoscaling, failure isolation।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Horizontal Scaling` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Horizontal scaling means adding more instances/nodes instead of making one machine bigger।
...
- business impact: এটি উন্নত করে capacity এবং resilience beyond vertical hardware limits।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Horizontal Scaling` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Horizontal Scaling` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Saying "just add more সার্ভারগুলো" যখন/একইসাথে keeping local session/স্টেট on each সার্ভার।
...
- `Horizontal Scaling` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
