...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি simplifies কনসিসটেন্সি এবং operations compared সাথে অ্যাক্টিভ-অ্যাক্টিভ, especially জন্য stateful সিস্টেমগুলো।
...
এই জায়গায় `Active-Passive` সামনে আসে। সহজ ভাষায়, Active-passive means one instance/region serves traffic while another stays ready to take over।
...
- Right: `Active-Passive` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Active-Passive` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Stateful primary/replica setups, strict কনসিসটেন্সি paths, simpler operations টিমগুলো।
- business signal: এটি simplifies কনসিসটেন্সি এবং operations compared সাথে অ্যাক্টিভ-অ্যাক্টিভ, especially জন্য stateful সিস্টেমগুলো।
...
`Active-Passive` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Ultra-low downtime requirements যেখানে failover delay হলো unacceptable এবং অ্যাক্টিভ-অ্যাক্টিভ হলো viable।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Forgetting to account জন্য passive capacity readiness এবং রেপ্লিকেশন lag।
...
- ধাপ ২: `Active-Passive` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Active-Passive` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: active, passive, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Active-Passive` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Active-passive means one instance/region serves traffic while another stays ready to take over।
...
- business impact: এটি simplifies কনসিসটেন্সি এবং operations compared সাথে অ্যাক্টিভ-অ্যাক্টিভ, especially জন্য stateful সিস্টেমগুলো।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Active-Passive` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Active-Passive` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Forgetting to account জন্য passive capacity readiness এবং রেপ্লিকেশন lag।
...
- `Active-Passive` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
