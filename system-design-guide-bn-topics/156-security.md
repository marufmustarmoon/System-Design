...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Scalable সিস্টেমগুলো become attack surfaces; সিকিউরিটি must হতে built into the design, না added later।
...
এই জায়গায় `Security` সামনে আসে। সহজ ভাষায়, Security patterns protect identity, access, data, and system boundaries in distributed architectures।
...
- Right: `Security` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Security` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: সবসময় জন্য production সিস্টেমগুলো, especially internet-facing এবং ডেটা-sensitive সার্ভিসগুলো।
- business signal: Scalable সিস্টেমগুলো become attack surfaces; সিকিউরিটি must হতে built into the design, না added later।
...
`Security` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না bolt on generic controls ছাড়া understanding threat model এবং ট্রাস্ট বাউন্ডারি।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Auth/Authz ছাড়া discussion জন্য a public API।
...
- ধাপ ২: `Security` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Security` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: authn/authz, trust boundary, least privilege, token scope, audit logs।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Security` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Security patterns protect identity, access, data, and system boundaries in distributed architectures।
...
- business impact: Scalable সিস্টেমগুলো become attack surfaces; সিকিউরিটি must হতে built into the design, না added later।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Security` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Security` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Auth/Authz ছাড়া discussion জন্য a public API।
...
- `Security` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
