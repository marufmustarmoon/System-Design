...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Interviewers test whether আপনি understand responsibilities এর বদলে ব্যবহার করে এগুলো terms interchangeably।
...
এই জায়গায় `LB vs Reverse Proxy` সামনে আসে। সহজ ভাষায়, A লোড ব্যালেন্সার distributes traffic across multiple backends।
...
- Right: `LB vs Reverse Proxy` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`LB vs Reverse Proxy` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: ব্যবহার LBs জন্য distribution; ব্যবহার reverse proxies জন্য policy, routing, caching, অথবা protocol handling।
- business signal: Interviewers test whether আপনি understand responsibilities এর বদলে ব্যবহার করে এগুলো terms interchangeably।
...
`LB vs Reverse Proxy` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না introduce multiple প্রক্সি layers সাথে overlapping responsibilities ছাড়া a reason।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Answering "they হলো the same thing" ছাড়া qualifying roles।
...
- ধাপ ২: `LB vs Reverse Proxy` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `LB vs Reverse Proxy` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: traffic distribution, request proxying, TLS termination, caching/rewrite, layer responsibility।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `LB vs Reverse Proxy` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: A লোড ব্যালেন্সার distributes traffic across multiple backends।
...
- business impact: Interviewers test whether আপনি understand responsibilities এর বদলে ব্যবহার করে এগুলো terms interchangeably।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `LB vs Reverse Proxy` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `LB vs Reverse Proxy` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Answering "they হলো the same thing" ছাড়া qualifying roles।
...
- `LB vs Reverse Proxy` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
