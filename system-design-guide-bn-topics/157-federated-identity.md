...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি centralizes অথেন্টিকেশন, উন্নত করে সিকিউরিটি posture, এবং সক্ষম করে SSO জুড়ে সিস্টেমগুলো।
...
এই জায়গায় `Federated Identity` সামনে আসে। সহজ ভাষায়, Federated identity lets applications trust an external identity provider (IdP) for authentication instead of managing user credentials directly।
...
- Right: `Federated Identity` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Federated Identity` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Enterprise সিস্টেমগুলো, multi-app ecosystems, SSO, partner integrations।
- business signal: এটি centralizes অথেন্টিকেশন, উন্নত করে সিকিউরিটি posture, এবং সক্ষম করে SSO জুড়ে সিস্টেমগুলো।
...
`Federated Identity` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Tiny standalone prototypes যেখানে IdP integration overhead হলো না justified।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Trusting টোকেনগুলো ছাড়া verifying signature, audience, এবং expiry।
...
- ধাপ ২: `Federated Identity` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Federated Identity` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: IdP, tokens/claims, SSO, token validation, authn vs authz।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Federated Identity` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Federated identity lets applications trust an external identity provider (IdP) for authentication instead of managing user credentials directly।
...
- business impact: এটি centralizes অথেন্টিকেশন, উন্নত করে সিকিউরিটি posture, এবং সক্ষম করে SSO জুড়ে সিস্টেমগুলো।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Federated Identity` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Federated Identity` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Trusting টোকেনগুলো ছাড়া verifying signature, audience, এবং expiry।
...
- `Federated Identity` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
