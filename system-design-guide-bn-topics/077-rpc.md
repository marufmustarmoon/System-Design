...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি simplifies সার্ভিস-টু-সার্ভিস integration এবং hides transport details behind typed interfaces।
...
এই জায়গায় `RPC` সামনে আসে। সহজ ভাষায়, RPC (Remote Procedure Call) is a style where a program calls a remote function/service as if calling a local procedure।
...
- Right: `RPC` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`RPC` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Internal microservices সাথে strong contracts, typed ক্লায়েন্টগুলো, এবং পারফরম্যান্স needs।
- business signal: এটি simplifies সার্ভিস-টু-সার্ভিস integration এবং hides transport details behind typed interfaces।
...
`RPC` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Public APIs requiring broad interoperability এবং browser-friendly semantics।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Treating RPC as automatically bad কারণ it হলো না REST।
...
- ধাপ ২: `RPC` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `RPC` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: protocol semantics, timeout/retry, versioning, idempotency, observability।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `RPC` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: RPC (Remote Procedure Call) is a style where a program calls a remote function/service as if calling a local procedure।
...
- business impact: এটি simplifies সার্ভিস-টু-সার্ভিস integration এবং hides transport details behind typed interfaces।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `RPC` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `RPC` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Treating RPC as automatically bad কারণ it হলো না REST।
...
- `RPC` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
