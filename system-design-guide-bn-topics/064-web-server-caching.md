...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি absorbs repeated রিকোয়েস্টগুলো ছাড়া hitting application code অথবা the ডাটাবেজ।
...
এই জায়গায় `Web Server Caching` সামনে আসে। সহজ ভাষায়, Web server caching stores responses or fragments at the web/reverse-proxy layer (e.g., NGINX, Varnish) before the app service।
...
- Right: `Web Server Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Web Server Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Hot public pages, fragment caching, origin protection।
- business signal: এটি absorbs repeated রিকোয়েস্টগুলো ছাড়া hitting application code অথবা the ডাটাবেজ।
...
`Web Server Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: Highly personalized endpoints ছাড়া safe ক্যাশ partitioning।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Duplicating CDN এবং প্রক্সি ক্যাশ rules ছাড়া understanding ক্যাশ ওনারশিপ।
...
- ধাপ ২: `Web Server Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Web Server Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Web Server Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Web server caching stores responses or fragments at the web/reverse-proxy layer (e.g., NGINX, Varnish) before the app service।
...
- business impact: এটি absorbs repeated রিকোয়েস্টগুলো ছাড়া hitting application code অথবা the ডাটাবেজ।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Web Server Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Web Server Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Duplicating CDN এবং প্রক্সি ক্যাশ rules ছাড়া understanding ক্যাশ ওনারশিপ।
...
- `Web Server Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
