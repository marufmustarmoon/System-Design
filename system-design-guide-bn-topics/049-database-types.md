...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Different ডেটা shapes এবং workloads require different storage ট্রেড-অফ।
...
এই জায়গায় `Database Types` সামনে আসে। সহজ ভাষায়, Database types classify storage systems by data model and query/access pattern (relational, key-value, document, graph, etc.)।
...
- Right: `Database Types` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Database Types` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: যখন mapping entities এবং access patterns to storage choices।
- business signal: Different ডেটা shapes এবং workloads require different storage ট্রেড-অফ।
...
`Database Types` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না choose a DB type by vendor familiarity alone।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Listing ডাটাবেজ types ছাড়া connecting them to queries।
...
- ধাপ ২: `Database Types` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Database Types` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: data model, query pattern, indexing, consistency, scale trade-off।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Database Types` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Database types classify storage systems by data model and query/access pattern (relational, key-value, document, graph, etc.)।
...
- business impact: Different ডেটা shapes এবং workloads require different storage ট্রেড-অফ।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Database Types` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Database Types` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Listing ডাটাবেজ types ছাড়া connecting them to queries।
...
- `Database Types` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
