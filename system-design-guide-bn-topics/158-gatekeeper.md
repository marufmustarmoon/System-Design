...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি centralizes অথেন্টিকেশন, অথরাইজেশন, validation, এবং policy enforcement at a ট্রাস্ট বাউন্ডারি।
...
এই জায়গায় `Gatekeeper` সামনে আসে। সহজ ভাষায়, Gatekeeper is a security pattern where a dedicated component controls and filters access to protected services/resources।
...
- Right: `Gatekeeper` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Gatekeeper` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Public APIs, admin interfaces, zero-ট্রাস্ট বাউন্ডারি, partner integrations।
- business signal: এটি centralizes অথেন্টিকেশন, অথরাইজেশন, validation, এবং policy enforcement at a ট্রাস্ট বাউন্ডারি।
...
`Gatekeeper` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না rely on gatekeeper শুধু এবং skip সার্ভিস-level অথরাইজেশন জন্য sensitive operations।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Internal সার্ভিসগুলো trust all ট্রাফিক from the গেটওয়ে ছাড়া verifying caller identity/claims।
...
- ধাপ ২: `Gatekeeper` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Gatekeeper` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: trust boundary, authn/authz, request validation, rate limiting, defense in depth।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Gatekeeper` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Gatekeeper is a security pattern where a dedicated component controls and filters access to protected services/resources।
...
- business impact: এটি centralizes অথেন্টিকেশন, অথরাইজেশন, validation, এবং policy enforcement at a ট্রাস্ট বাউন্ডারি।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Gatekeeper` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Gatekeeper` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Internal সার্ভিসগুলো trust all ট্রাফিক from the গেটওয়ে ছাড়া verifying caller identity/claims।
...
- `Gatekeeper` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
