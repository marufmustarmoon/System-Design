...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি সাহায্য করে আপনি choose the right ক্যাশ জন্য the right bottleneck এর বদলে defaulting to one tool।
...
এই জায়গায় `Types of Caching` সামনে আসে। সহজ ভাষায়, "Types of caching" is a way to classify ক্যাশs by location, data granularity, and policy (client/CDN/proxy/app/DB; object/query/page/fragment; write/read strategies)।
...
- Right: `Types of Caching` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Types of Caching` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: সময় ট্রেড-অফ discussions এবং architecture decomposition।
- business signal: এটি সাহায্য করে আপনি choose the right ক্যাশ জন্য the right bottleneck এর বদলে defaulting to one tool।
...
`Types of Caching` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না spend too much time naming ক্যাশ categories যদি the core issue হলো an unindexed query।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Treating ক্যাশ type selection as a substitute জন্য workload analysis।
...
- ধাপ ২: `Types of Caching` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Types of Caching` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: cache key, TTL, invalidation, cache hit ratio, stale data।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Types of Caching` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: "Types of caching" is a way to classify ক্যাশs by location, data granularity, and policy (client/CDN/proxy/app/DB; object/query/page/fragment; write/read strategies)।
...
- business impact: এটি সাহায্য করে আপনি choose the right ক্যাশ জন্য the right bottleneck এর বদলে defaulting to one tool।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Types of Caching` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Types of Caching` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Treating ক্যাশ type selection as a substitute জন্য workload analysis।
...
- `Types of Caching` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
