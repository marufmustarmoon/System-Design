...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, এটি কমায় downtime যখন a primary সার্ভার, ডাটাবেজ, অথবা রিজিয়ন becomes unavailable।
...
এই জায়গায় `Fail-Over` সামনে আসে। সহজ ভাষায়, Failover is switching traffic from a failed component to a healthy backup component।
...
- Right: `Fail-Over` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Fail-Over` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Single-primary DBs, regional সার্ভিসগুলো, অথবা critical সার্ভিসগুলো সাথে standby capacity।
- business signal: এটি কমায় downtime যখন a primary সার্ভার, ডাটাবেজ, অথবা রিজিয়ন becomes unavailable।
...
`Fail-Over` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: যদি backup instances হলো cold এবং recovery time হলো longer than acceptable SLO।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: Assuming failover হলো instant এবং lossless জন্য stateful সিস্টেমগুলো।
...
- ধাপ ২: `Fail-Over` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Fail-Over` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: fail, over, use case, trade-off, failure case।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Fail-Over` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Failover is switching traffic from a failed component to a healthy backup component।
...
- business impact: এটি কমায় downtime যখন a primary সার্ভার, ডাটাবেজ, অথবা রিজিয়ন becomes unavailable।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Fail-Over` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Fail-Over` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: Assuming failover হলো instant এবং lossless জন্য stateful সিস্টেমগুলো।
...
- `Fail-Over` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
