...
মুন মিয়াঁর টিম প্রোডাক্ট launch করার পর দেখল, Overload পারে cause cascading ফেইলিউরগুলো এবং lower overall অ্যাভেইলেবিলিটি জন্য everyone।
...
এই জায়গায় `Throttling` সামনে আসে। সহজ ভাষায়, Throttling limits request rates or resource usage to protect a system from overload।
...
- Right: `Throttling` নিলে কোন metric ভালো হবে (latency/error/cost) আর কোন complexity বাড়বে, আগে থেকেই লিখে রাখা
...
`Throttling` decision-making-কে concrete করে: abstract theory থেকে সরাসরি architecture action-এ নিয়ে আসে।
...
- strongest signal: Public APIs, shared সার্ভিসগুলো, dependency protection, multi-tenant fairness।
- business signal: Overload পারে cause cascading ফেইলিউরগুলো এবং lower overall অ্যাভেইলেবিলিটি জন্য everyone।
...
`Throttling` ভুল context-এ নিলে solution-এর বদলে নতুন incident তৈরি করে।
- wrong context: করবেন না apply one hard limit জন্য all ক্লায়েন্টগুলো যদি priorities/SLAs differ।
- misuse করলে latency বেড়ে যেতে পারে, stale/incorrect output আসতে পারে, বা retry cascade তৈরি হতে পারে।
- interview red flag: কোনো throttling on public অথবা multi-tenant APIs।
...
- ধাপ ২: `Throttling` design-এর invariant লিখুন: কোনটা ভাঙা যাবে না, কোনটা degrade হতে পারে।
...
- `Throttling` হলো এমন একটি design lens, যা business requirement আর system behavior-কে একই ফ্রেমে আনে।
- Interview keywords: rate limiting, load shedding, fairness, abuse protection, back pressure।

## এটা কী (থিওরি)

- বাংলা সারাংশ: `Throttling` কেবল সংজ্ঞা না; এটি problem-context অনুযায়ী সঠিক guarantee ও architecture boundary বেছে নেওয়ার কৌশল।
- সহজ সংজ্ঞা: Throttling limits request rates or resource usage to protect a system from overload।
...
- business impact: Overload পারে cause cascading ফেইলিউরগুলো এবং lower overall অ্যাভেইলেবিলিটি জন্য everyone।
- এই design না থাকলে short-term patch জমতে জমতে সিস্টেম brittle হয়ে যায়।

## কীভাবে কাজ করে (সিনিয়র-লেভেল ইনসাইট)

- সিনিয়র দৃষ্টিতে `Throttling` কাজ করে clear boundary তৈরির মাধ্যমে: data path, control path, failure path আলাদা করা হয়।
...
- তাই `Throttling` implement করার আগে traffic shape, state model, dependency graph, আর blast radius map করা জরুরি।
...
- red flag avoid করুন: কোনো throttling on public অথবা multi-tenant APIs।
...
- `Throttling` বাছাই করবেন requirement-fit দেখে, trend দেখে না।
